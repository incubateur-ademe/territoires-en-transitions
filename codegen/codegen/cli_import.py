...
from codegen.generated.referentiels import actions
...
def dteci(
    ...
    """
    eci = next(
        action for action in actions if action.id.startswith("economie_circulaire")
    )
    table_correspondance: list = load_json(
        os.path.join(data_dir, "table_correspondance.json")
    )
    server_niveaux = load_json(os.path.join(data_dir, "server_niveaux.json"))
    server_territoires: Dict[dict] = load_json(
        os.path.join(data_dir, "server_territoires.json")
    )
    collectivites = pd.read_excel(
        os.path.join(data_dir, "collectivités.xlsx"), dtype=str, sheet_name=0, header=0
    )

    nom_repris: List[str] = [nom]

    territoires_repris: List[dict] = [
        territoire
        for territoire in server_territoires.values()
        if str(territoire["nom"]).strip() in nom_repris
    ]

    id_repris: List[str] = [t["id"] for t in territoires_repris]
    niveaux_repris: List[dict] = [
        n for n in server_niveaux.values() if n["territoireId"] in id_repris
    ]
    ...
    def action_by_id(
        action_id: str, actions: List[ActionReferentiel]
    ) -> ActionReferentiel:
    ...
    for axe in table_correspondance:
        for orientation in axe["orientations"]:
            for niveau in orientation["niveaux"]:
                ...
                niveau_data = [
                    n for n in niveaux_repris if n["niveauId"] == niveau["id"]
                ]
                ...
                if question := indicateur.get("question"):
                    ...
                    if (
                        not oui["faite"]
                        and not oui["pas_faite"]
                        and not non["faite"]
                        and not non["pas_faite"]
                    ):
                        # no way to extract statuts
                        for n in niveau_data:
                            if valeur := n.get("valeur"):
                                add_message(
                                    f"Votre réponse : {'oui' if valeur['oui'] else 'non'}",
                                    n,
                                )
                ...
                # choix (checkboxes)
                    ...
                    taches_ids = [
                        action.id_nomenclature for action in niveau_action.actions
                    ]
                    data_by_territoire = {}
                    for n in niveau_data:
                    ...
                    for territoire_data in data_by_territoire.values():
                        for yearly_data in territoire_data:
                            chosen = [
                                n["id"]
                                for n in yearly_data.get("valeur", {}).get("ids", [])
                                if n
                            ]
                            faites = list(
                                itertools.chain.from_iterable(
                                    [c["faite"] for c in choix if c["id"] in chosen]
                                )
                            )
                ...
                # liste (dropdown)
                    ...
                    for n in niveau_data:
                        if valeur := n.get("valeur"):
                            selected = valeur["id"]
                            match = next(
                                item for item in liste if item["id"] == selected
                            )
    ...
    for statut in statuts:
        ...
        latest_statuts.append(
            sorted(
                [
                    s
                    for s in statuts
                    if s.epci_id == statut.epci_id and s.action_id == statut.action_id
                ],
    ...
    meta_sql = "\n".join(
        [
            line
            for line in [data_to_meta_insert(data) for data in niveaux_repris]
            if line
        ]
    )
    epci_sql = "\n".join(
        [territoire_to_epci_insert(territoire) for territoire in territoires_repris]
    )
    statuts_sql = "\n".join(
        [statut_to_statut_insert(statut) for statut in latest_statuts]
    )
...
Vos données à actualiser manuellement dans votre référentiel sur Territoires en Transitions :
    ...
    for territoire in territoires_repris:
        ...
        text = "\n\n".join(
            f"Niveau {msg.action_id}, année {msg.annee}:\n{msg.body}" for msg in msgs
        )
        ...
        try:
            infos = next(
                f"{row['prénom']} {row['nom.1']} <{row['email']}>"
                for index, row in collectivites.iterrows()
                if str(row["nom"]).strip() == territoire.get("nom", "").strip()
            )

            write(
                os.path.join(output_dir, dirname, "mail.txt"),
                infos + "\n" + mail + "\n\n" + text,
            )
        except:
                ...
                for niveau in orientation["niveaux"]:
                    ...
                    if "question" in indicateur.keys():
                        ...
                        if action["actions"]:
                            ...
                            indicateur[
                                "raison"
                            ] = f"{len(action['actions'])} actions pour un seul niveau en oui non"
                            continue
                    ...
                    # handle many oui non
                        ...
                        if not action["actions"]:
                            indicateur[
                                "raison"
                            ] = "Pas de sous actions pour plusieurs oui non"
                            continue
                    ...
                    # handle interval
                        ...
                        indicateur[
                            "raison"
                        ] = "Pas de correspondance pour des intervalles de valeurs"
                    ...
                    # handle checkboxes
                        ...
                        if not action["actions"]:
                            indicateur[
                                "raison"
                            ] = f"pas de sous actions à {action['id']} pour ce niveau à choix multiple"
                            continue

                        if len(choix) > len(action["actions"]):
                            indicateur[
                                "raison"
                            ] = f"plus d'options ({len(choix)}) que d'actions ({len(action['actions'])})"
                            continue
                        ...
                        for option in choix:
                            ...
                            option[
                                "raison"
                            ] = f'"{action["id"]} {option["nom"]}" ressemble à {score}% à "{choice}"'
                    ...
                    # handle dropdown
                        ...
                        if not action["actions"]:
                            indicateur[
                                "raison"
                            ] = f"pas de sous actions à {action['id']} pour ce niveau à liste"
                            continue

                        if len(liste) > len(action["actions"]):
                            indicateur[
                                "raison"
                            ] = f"plus d'options ({len(liste)}) que d'actions ({len(action['actions'])})"
                            continue
                        ...
                        for option in liste:
                            ...
                            option["faite"] = [chosen["id"]]
                            option["faite"] = [
                                f"{parentId(chosen)}.{n}" for n in range(1, i + 1)
                            ]
                            option[
                                "raison"
                            ] = f'"{action["id"]} {option["nom"]}" ressemble à {score}% à "{choice}"'
