...
def parse_referentiel_points(referentiel: str) -> dict:
    ...
    for index, row in calculs.iterrows():
        points[row["orientation"]] = points_from_ponderation(
            row["pondération_orientation"]
        )

        for niveau in niveaux:
            if not stripped(row[f"niveau_{niveau}"]):
                continue
            points[f"{row['orientation']}.{niveau}"] = points_from_ponderation(
                row[f"niveau_{niveau}"]
            )
        ...
        for index, row in axe.iterrows():
            ...
            if orientation_n:
                orientation = {
                    ...
                    "points": points[orientation_n]
                    if orientation_n in points.keys()
                    else "",
                ...
                row["orientation_description"]
            ):  # sometime description is not on the same row
                orientation["description"] = orientation["description"] + stripped(
                    row["orientation_description"]
                )

            if stripped(row["description"]) and stripped(
                row["description"]
