...
describe('CollectiviteBloc', () => {
  it('should change to Collectivite given siren when connected user is referent, so editable', () => {
    const currentCollectiviteBloc = new CurrentCollectiviteBloc();

    supabase.auth.signIn({email: 'yolo@dodo.com', password: 'yolododo'});
    currentCollectiviteBloc.update({id: 1});
  ...
  it('should change to Collectivite given siren when connected user does not belong to Collectivite, so not editable', () => {
    const currentCollectiviteBloc = new CurrentCollectiviteBloc();
