...
import { CollectiviteTypeField } from '@/app/app/pages/Support/AjouterCollectivite/collectivite-type.field';
...
import {
  CollectiviteInput,
  useCompleteCollectivite,
} from '@/app/app/pages/Support/AjouterCollectivite/use-complete-collectivite';
...
export const AjouterCollectivitePage = () => {
  ...
  const { refetch: searchInsee } = useCompleteCollectivite(collectivite);
  const { mutate: createCollectivite, isPending } = useSaveCollectivite();
  ...
  const updateCollectivite = (
    ...
    setCollectivite((prev) => {
      ...
      if (key === 'typeId') {
        ...
        return { typeId: value ?? 'commune' };
      }
      if (reset) {
        setStatus(statusSearch.none);
        return { typeId: collectivite.typeId, [key]: value };
      }
      return {
        ...prev,
  ...
  const handleSave = () => {
    if (collectivite.nom) {
      const reponse = createCollectivite(collectivite, {
        onSuccess: (data) => {
          setMessage({
    ...
    <PageContainer>
      ...
      <div className="flex items-start gap-4">
        <CollectiviteTypeField
          type={collectivite.typeId ?? 'commune'}
          onSelect={(type) =>
            updateCollectivite('typeId', type?.value as string)
          }
        />
        {collectivite.typeId == 'commune' && (
          <CodeCommuneField
            value={collectivite.communeCode ?? ''}
            onChange={(value) => updateCollectivite('communeCode', value, true)}
          />
        )}
        {collectivite.typeId == 'epci' && (
          <SirenField
            value={collectivite.siren ?? ''}
            onChange={(value) => updateCollectivite('siren', value, true)}
          />
        )}
        {collectivite.typeId == 'departement' && (
        ...
        {collectivite.typeId && collectivite.typeId != 'test' && (
          <Button
            ...
            disabled={
              ...
              (collectivite.typeId == 'region' && !collectivite.regionCode)
            }
      ...
      <div className="space-y-6">
        {status === statusSearch.none && collectivite.typeId !== 'test' && (
        ...
        <Field title="Population">
          <InputNumber
            value={collectivite.population ?? ''}
            onChange={(e) => updateCollectivite('population', e.target.value)}
          />
        </Field>
        <div className="flex items-start gap-4">
          <Button
            ...
            disabled={
              ...
              (collectivite.typeId === 'epci' && !collectivite.siren) ||
              (collectivite.typeId !== 'region' &&
                collectivite.typeId !== 'test' &&
                !collectivite.departementCode) ||
              (collectivite.typeId !== 'test' && !collectivite.regionCode)
            }
            onClick={handleSave}
          >
