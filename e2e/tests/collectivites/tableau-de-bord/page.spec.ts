...
testWithUsers.describe('accès aux pages du tableau de bord', () => {
  testWithUsers("Page d'accueil après le login tableau de bord synthetique", async ({ page, users }) => {
    ...
    await page.goto(`/collectivite/${collectivite.id}/tableau-de-bord/synthetique`);


    await expect(page.getByRole('heading', { name: `Tableau de bord de la collectivité ${collectivite.nom}` })).toBeVisible();
