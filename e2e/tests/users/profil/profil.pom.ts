...
export class ProfilPom {
  ...
  async submitForm() {
    // Wait for the tRPC mutation to complete
    const responsePromise = this.page.waitForResponse(
      (response) =>
        response.url().includes('/trpc/users.update') &&
        response.request().method() === 'POST'
    );
    await this.modalSubmitButton.click();
    await expect(this.modal).toBeHidden();
    await responsePromise;
    // Wait a bit for the UI to update after the mutation
  ...
  async expectTelephone(expected: string) {
    await expect(this.telephoneDisplay).toHaveText(expected);
  }

  async expectTelephoneNotSet() {
    await expect(this.telephoneDisplay).toHaveText('Non renseigné');
  }
}
