...
Deno.test("Création fiches et plan actions", async () => {
  ...
  const fiche = {
    ...
    piliers_eci: [
      "Écoconception" as Database["public"]["Enums"][
  ...
  await supabase.rpc("ajouter_thematique", {
    "fiche_id": fId,
    thematique: thematiques.data![0].thematique,
  });
  await supabase.rpc("ajouter_thematique", {
    "fiche_id": fId,
    thematique: thematiques.data![1].thematique,
  });
  ...
  await supabase.rpc("ajouter_sous_thematique", {
    "fiche_id": fId,
    thematique_id: sousThematiques.data![0].id,
  });
  ...
  const plansActionColData = plansActionCol
    .data! as Database["public"]["Tables"]["axe"]["Row"][];
  ...
  const partenaireSave = insertPartenaire
    .data! as unknown as Database["public"]["Tables"]["partenaire_tag"]["Row"];
  ...
  const partTags = await supabase.from("fiche_action_partenaire_tag")
    ...
    .eq("fiche_id", fId);
  const partTagsData = partTags.data![0]
    .partenaire_tag as Database["public"]["Tables"]["partenaire_tag"]["Row"][];
  // Enlever partenaire de la fiche
  const enlever = await supabase.rpc("enlever_partenaire", {
    "fiche_id": fId,
    "partenaire": partenaireSave,
  });
  ...
  await supabase.rpc("enlever_structure", {
    "fiche_id": fId,
    "structure": insertStructure.data!,
  });
  ...
  await supabase.rpc("ajouter_pilote", {
    "fiche_id": fId,
    "pilote": piloteUtilisateur,
  });
  // Enlever pilote
  await supabase.rpc("enlever_pilote", {
    "fiche_id": fId,
    "pilote": insertPiloteTag.data!,
  });
  ...
  await supabase.rpc("ajouter_referent", {
    "fiche_id": fId,
    "referent": referentUtilisateur,
  });
  // Enlever referent
  await supabase.rpc("enlever_referent", {
    "fiche_id": fId,
    "referent": insertReferentTag.data!,
  });
  ...
  const annexe = {
    collectivite_id: 1,
  } as Database["public"]["Tables"]["annexe"]["Insert"];
  ...
  const thematiquesVue = vue.data![0]
    .thematiques! as Database["public"]["Tables"]["thematique"]["Row"][];
  const sousThematiquesVue = vue.data![0]
    .sous_thematiques! as Database["public"]["Tables"]["sous_thematique"][
      "Row"
    ][];
  const partenairesVue = vue.data![0]
    .partenaires! as Database["public"]["Tables"]["partenaire_tag"]["Row"][];
  const structuresVue = vue.data![0]
    .structures! as Database["public"]["Tables"]["structure_tag"]["Row"][];
  ...
  const annexesVue = vue.data![0]
    .annexes! as Database["public"]["Tables"]["action_relation"]["Row"][];
  const actionVue = vue.data![0]
    .actions! as Database["public"]["Tables"]["annexe"]["Row"][];
  const axesVue = vue.data![0]
    .actions! as Database["public"]["Tables"]["axe"]["Row"][];
  ...
  const ficheVue = {
    ...
    objectifs: "verif",
    resultats_attendus: [
      "Sensibilisation" as Database["public"]["Enums"][
        "fiche_action_resultats_attendus"
      ],
    ],
    cibles: [
      "Grand public et associations" as Database["public"]["Enums"][
        "fiche_action_cibles"
      ],
    ],
    ressources: null,
    ...
    niveau_priorite:
      "Bas" as Database["public"]["Enums"]["fiche_action_niveaux_priorite"],
    date_debut: null,
  ...
  const check = await supabase.from("fiches_action").update(ficheVue as never)
    .select();
...
Deno.test("Création d'une fiche en utilisant la vue", async () => {
  ...
  const insertResponse = await supabase.from("fiches_action")
...
Deno.test("Plan d'actions d'une collectivité", async () => {
  ...
  const selectResponse1 = await supabase.from("axe").select().eq(
    "collectivite_id",
    1,
  ).is("parent", null);
  assertExists(selectResponse1.data);
