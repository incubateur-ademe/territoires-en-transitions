...
type TypePlan = Database["public"]["Tables"]["plan_action_type"]["Row"];
type PlanType = Axe & {
  type?: TypePlan;
};

Deno.test(
  "Plans d'une collectivité avec leurs types",
  { ignore: true },
  async () => {
    // await testReset();
    await signIn("yolododo");

    const select = await supabase.from("axe")
...
Deno.test("Test type", async () => {
  ...
  const plan = await supabase.from("axe")
    .update({ type: type.id })
    .eq("id", 1)
    .select();
...
Deno.test("Test plan", async () => {
  ...
  // Ajouter un plan
  const planInsert = await supabase.from("axe")
    .insert({ collectivite_id: 1, nom: "Plan Test" })
    .select();
  const plan: Database["public"]["Tables"]["axe"]["Row"] = planInsert.data![0];
  assertEquals(plan.id, plan.plan);

  // Ajouter un axe
  const axeInsert = await supabase.from("axe")
    .insert({ collectivite_id: 1, nom: "Axe Test", parent: plan.id })
    .select();
