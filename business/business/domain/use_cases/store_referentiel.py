...
from business.domain.ports.action_definition_repo import AbstractActionDefinitionRepository

class StoreReferentiel:
    def __init__(self, 
    bus: AbstractDomainMessageBus,
    points_repo: AbstractActionPointsRepository, 
    children_repo: AbstractActionChildrenRepository, 
    definition_repo: AbstractActionDefinitionRepository,
    ) -> None:
        self.bus = bus 
    ...
    def execute(self, command: commands.StoreReferentielEntities):
        ...
        try: 
            self.points_repo.add_entities(self.points_entities)
            self.children_repo.add_entities(self.children_entities)
            self.definition_repo.add_entities(self.definition_entities)
            self.bus.publish_event(events.ReferentielStored(referentiel_id=command.referentiel_id))
    ...
    def fail_if_entities_are_inconsistent(self) -> Optional[events.DomainFailureEvent]:
        ...
        if not (len(self.definition_entities) == len(self.children_entities) == len(self.points_entities)):
            return events.ReferentielStorageFailed(f"Entity count are different between definitions ({len(self.definition_entities)}), points ({len(self.points_entities)}) and children ({len(self.children_entities)})!")
        ...
        if not (set(def_action_ids) == set(children_action_ids) == set(points_action_ids)):
            return events.ReferentielStorageFailed(f"Not same action ids between definitions ({def_action_ids}), points ({points_action_ids}) and children ({children_action_ids})!")

        # check action ids in children are coherent
        for children_entity in self.children_entities:
            if not all([child_id in children_action_ids for child_id in children_entity.children_ids]):
                return events.ReferentielStorageFailed(f"Inconsistency in action {children_entity.action_id}: some children id are refered but defined.")