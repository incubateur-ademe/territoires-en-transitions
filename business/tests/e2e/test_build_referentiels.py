...
def test_build_economie_circulaire():
    ...
    parsing_failure_events: List[
        events.ParseMarkdownReferentielFolderFailed
    ] = spy_on_event(bus, events.ParseMarkdownReferentielFolderFailed)
    folder_parsed_events: List[events.MarkdownReferentielFolderParsed] = spy_on_event(
        bus, events.MarkdownReferentielFolderParsed
    )
    ...
    command = commands.ConvertMarkdownReferentielNodeToEntities(
        referentiel_node=folder_parsed_events[0].referentiel_node
    )

    markdown_converted_events: List[
        events.MarkdownReferentielNodeConvertedToEntities
    ] = spy_on_event(bus, events.MarkdownReferentielNodeConvertedToEntities)
    inconsistency_found_events: List[
        events.MarkdownReferentielNodeInconsistencyFound
    ] = spy_on_event(bus, events.MarkdownReferentielNodeInconsistencyFound)
    ...
    assert (
        len(markdown_converted_events[0].definitions)
        == len(markdown_converted_events[0].definitions)
        == expected_nb_of_action_in_eci
    )
