...
const HeaderBody = ({
  ...
  return (
    <div className="w-full mx-auto py-4 xl:max-w-7xl xl:px-2 flex items-center justify-between">
      {/* Contenu principal */}
      <div className="max-lg:w-full max-lg:flex-wrap flex gap-x-4">
        <div className="max-lg:py-2 max-lg:w-full max-lg:px-4 max-lg:border-b border-b-primary-4 flex justify-between">
          {/* Logos */}
          {!!logos && (
        ...
        {/* Titres */}
        <div className="max-lg:w-full max-lg:pt-3 max-lg:mx-3 lg:p-4 flex flex-col justify-center">
      ...
      <div className="max-lg:hidden">
        <ul className="list-none flex gap-4 divide-x divide-x-grey-4 mb-0">
          {quickAccessButtons?.({
            variant: 'underlined',
            size: 'sm',
            className: 'border-b-0 hover:border-b',
          }).map((button, idx) => (
            <li key={idx} className="pl-4 first-of-type:pl-0 pb-0">
              {button}
            </li>
          ))}
        </ul>
