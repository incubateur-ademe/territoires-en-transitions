...
/** Pièce réellement déposée pour une démarche. */
...
/** Déclaration de couverture d'une pièce sans dépôt de document. */
export const demarcheDocumentCouvertureSchema = z.object({
  documentId: z.string(),
  source: demarcheDocumentCouvertureSourceSchema,
});

export type DemarcheDocumentCouverture = z.infer<
  typeof demarcheDocumentCouvertureSchema
>;
...
export const demarcheDocumentsSnapshotSchema = z.object({
  ...
  couvertures: z.array(demarcheDocumentCouvertureSchema),
  /** Un plan d'actions est rattaché à la démarche (conditionne la couverture `plan_actions`). */
  planActionRattache: z.boolean(),
});
