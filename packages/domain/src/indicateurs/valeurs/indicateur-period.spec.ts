...
describe('IndicateurPeriods', () => {
  ...
  it('refuse une configuration dont le codec ne peut pas identifier chaque période', () => {
    expect(() =>
      ...
      IndicateurPeriodErrorEnum.INDICATEUR_PERIOD_SERIALIZATION_INCOMPATIBLE
    );
  });
  ...
  it('refuse un ancrage mensuel qui ne commence pas le premier jour', () => {
    ...
    ).toThrow(IndicateurPeriodErrorEnum.INDICATEUR_CALENDAR_ANCHOR_INVALID);
  });
  ...
  it('expose un parseur sûr pour les clés non fiables', () => {
    ...
    expect(IndicateurPeriods.safeFromKey('inconnue:2026')).toMatchObject({
      ...
      error: expect.objectContaining({
        code: IndicateurPeriodErrorEnum.INDICATEUR_PERIODICITE_UNKNOWN,
        details: { periodicite: 'inconnue' },
      }),
    });
  });
  ...
  it('valide et réhydrate la représentation JSON', () => {
    ...
    const invalid = indicateurPeriodSchema.safeParse({
      periodicite: 'annuelle',
      dateDebut: '2026-02-01',
    });
      ...
      expect(invalid.error.issues).toEqual([
        expect.objectContaining({
          ...
          params: {
            ...
            details: { periodicite: 'annuelle', dateValeur: '2026-02-01' },
          },
        }),
      ]);
    }
  ...
  it('refuse de comparer silencieusement deux cadences différentes', () => {
    ...
    expect(() => IndicateurPeriods.compare(annual, monthly)).toThrow(
      expect.objectContaining({
        code: IndicateurPeriodErrorEnum.INDICATEUR_PERIODICITE_MISMATCH,
        details: { expected: 'annuelle', actual: 'mensuelle' },
      })
    );
  ...
  it('refuse les déplacements fractionnaires et les dépassements du calendrier', () => {
    ...
    expect(() => IndicateurPeriods.add(first, 0.5)).toThrow(
      IndicateurPeriodErrorEnum.INDICATEUR_PERIOD_OFFSET_INVALID
    );
    expect(() => IndicateurPeriods.add(first, -1)).toThrow(
      IndicateurPeriodErrorEnum.INDICATEUR_CALENDAR_OUT_OF_RANGE
    );
    expect(() => IndicateurPeriods.next(last)).toThrow(
      IndicateurPeriodErrorEnum.INDICATEUR_CALENDAR_OUT_OF_RANGE
    );
  });
});
