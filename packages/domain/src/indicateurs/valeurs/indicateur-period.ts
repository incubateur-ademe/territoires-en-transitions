...
const periodStrategies = Object.freeze({
  [IndicateurPeriodiciteEnum.ANNUELLE]: createCalendarMonthStrategy({
...
const assertPeriodPeriodicite: <K extends IndicateurPeriodicite>(
  ...
  if (period.periodicite !== periodicite) {
    throw createIndicateurPeriodError({
      ...
      details: { expected: periodicite, actual: period.periodicite },
    });
  }
};
...
const fromKey = (value: string): IndicateurPeriod => {
  ...
  if (separatorIndex < 1) {
    throw createIndicateurPeriodError({
      code: IndicateurPeriodErrorEnum.INDICATEUR_PERIOD_KEY_INVALID,
      details: { value },
    });
  }
  ...
  z.transform((value, context): IndicateurPeriod => {
    try {
      ...
      context.issues.push({
        ...
        ...(isIndicateurPeriodError(error)
          ? { params: { code: error.code, details: error.details } }
          : {}),
      });
      return z.NEVER;
    }
