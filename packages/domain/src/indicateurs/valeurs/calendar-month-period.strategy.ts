...
export const createCalendarMonthStrategy = <K extends IndicateurPeriodicite>({
  ...
  if (
    ...
    throw createIndicateurPeriodError({
      code: IndicateurPeriodErrorEnum.INDICATEUR_CALENDAR_STEP_INVALID,
      details: { periodicite, monthsPerPeriod },
    });
  }
  ...
  if (anchor.day !== 1) {
    throw createIndicateurPeriodError({
      code: IndicateurPeriodErrorEnum.INDICATEUR_CALENDAR_ANCHOR_INVALID,
      details: { periodicite, anchorDate },
    });
  }
  if (
    serialization === 'year' &&
    ...
    throw createIndicateurPeriodError({
      ...
      details: { periodicite, monthsPerPeriod, anchorDate, serialization },
    });
  }
  ...
  const add = (
    ...
    if (!Number.isInteger(amount)) {
      throw createIndicateurPeriodError({
        code: IndicateurPeriodErrorEnum.INDICATEUR_PERIOD_OFFSET_INVALID,
        details: { amount },
      });
    }
    const validatedPeriod = fromDateValeur(period.dateDebut);
