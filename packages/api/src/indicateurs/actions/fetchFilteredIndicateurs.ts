...
import {IndicateurListItem} from "../domain";
...
const filtresOptions: {[key in keyof Filters]?: string} = {
  ...
  plan_ids: 'fiche_action_indicateur!inner(fiche_id, fiche_action!inner(fiche_action_axe!inner(axe!inner())))',
  ...
  confidentiel : 'indicateur_collectivite(commentaire, confidentiel, collectivite_id)',
  fiches_non_classees: 'fiche_action_indicateur!inner(fiche_id, fiche_action!inner(fiche_action_axe!inner(axe!inner())))'
};
...
export async function fetchFilteredIndicateurs(
    ...
    subset: Subset | null,
    filters : Filters
) :Promise<{data:IndicateurListItem[], status:any}> {
    ...
    if(subset!== null ||filters.type?.length){
      ...
      if(subset !== null){
        if (filters.type?.length) {
          ...
          query.or(`nom.eq.${subsetPassage.get(subset)!}, nom.in.${filters.type}`,
              {referencedTable : 'indicateur_categorie_tag.categorie_tag'});
        }else {
          // S'il y a un tri que sur le programme
          query.eq('indicateur_categorie_tag.categorie_tag.nom', subsetPassage.get(subset)!);
        }
  ...
  if(filters.confidentiel===false){
    // Filtre supplémentaire sur confidentiel car pas trouvé comment faire ma condition avec postgrest
      rows = rows.filter((r: any)=>
          r.indicateur_collectivite.length === 0 || r.indicateur_collectivite[0].confidentiel === false);
  }
  // Filtre sur les indicateurs d'un groupement
  rows = rows.filter((r : any)=> r.groupement_id === null ||
  groupements.filter((g : Groupement) => g.id === r.groupement_id)[0].collectivites?.includes(collectiviteId));
  return {
    ...remaining,
    data: rows
        // et conserve un id unique
        .map((d : any) => ({id : d.id as number, titre : d.titre as string}))
  };
}
