...
export async function selectIndicateurSources (
    ...
    const {data, error} = await dbClient
        ...
        .select('...indicateur_source_metadonnee(...indicateur_source(*))')
        .eq('indicateur_id', indicateurId)
        .eq('collectivite_id', collectiviteId)
        .returns<Source[]>();

    return data?.filter((s : Source) => s.id !== null)
        .reduce((uniques, s) => {
            // Enlever les doublons en utilisant id comme clé
            if (!uniques.some(unique => unique.id === s.id)) {
                uniques.push(s);
            }
            return uniques;
        }, [] as Source[]) || [];
...
export async function selectIndicateurPilotes(
    ...
    const {data, error} = await dbClient
        ...
        .select(`id, collectivite_id, user_id, tag_id, tag:personne_tag(*), user:indicateur_pilote_user(*)`)
        .eq('indicateur_id', indicateurId)
        .eq('collectivite_id', collectiviteId)
        .returns<any[]>();
 ...
 */
export async function selectIndicateurThematiques(dbClient : DBClient, indicateurId : number) : Promise<number[]> {
    const {data, error} = await dbClient
 ...
 */
export async function selectIndicateurActions(dbClient : DBClient, indicateurId : number) : Promise<Action[]> {
    const {data, error} = await dbClient
...
export async function selectIndicateurListItems(
    ...
    const query =  dbClient
        ...
        .select('id, titre, groupement_id, plus:indicateur_collectivite(collectivite_id)');
    ...
    if(predefini){
        ...
        dataFilter = dataFilter.filter((item : any)=>
            item.groupement_id === null || groupementIds.includes(item.groupement_id));
    }
    const toReturn = dataFilter.map((item : any)=>{
        const plusInfoFilter =
            item.plus?.filter((i : any) => i.collectivite_id === collectiviteId) || [];
        const plusInfo = plusInfoFilter && plusInfoFilter[0] ? plusInfoFilter[0] :
...
export async function selectIndicateurValeurs(
    ...
    if(!source) {
        ...
        toReturn = data?.filter((val: any) => val.source === null || val.source.length === 0) || [];
    }else{
        // Récupère les valeurs de la source donnée (garde que la dernière si conflit sur la date)
        const valeursSource = data?.filter((val: any) =>
            val.source &&
            val.source.source_id === source
        ) || [];
        ...
        const groupeParAnneeEstimation : Map<number, any[]> = new Map<number, any[]>();
        valeursSource.forEach((val : any) => {
            ...
            if(val.resultat || val.resultat_commentaire){
                if (!groupeParAnneeResultat.get(annee)) groupeParAnneeResultat.set(annee, []);
                groupeParAnneeResultat.get(annee)!.push(val);
            }
            if(val.objectif || val.objectif_commentaire){
                if (!groupeParAnneeObjectif.get(annee)) groupeParAnneeObjectif.set(annee, []);
                groupeParAnneeObjectif.get(annee)!.push(val);
            }
            if(val.estimation){
                if (!groupeParAnneeEstimation.get(annee)) groupeParAnneeEstimation.set(annee, []);
                groupeParAnneeEstimation.get(annee)!.push(val);
            }

        });
        const maps = [groupeParAnneeResultat, groupeParAnneeObjectif, groupeParAnneeEstimation];
        toReturn =  [];
        const ids = [];
        for (let m = 0;  m<maps.length; m++){
            const map = maps[m];
            for (const annee of map.keys()) {
                const plusRecente = (map.get(annee)!).reduce((a, b) =>
                    new Date(a.source.date_version) > new Date(b.source.date_version) ? a : b
                );
 ...
 * @return valeur
 */
export async function selectIndicateurValeur(dbClient: DBClient, valeurId : number) : Promise<Valeur | null> {
    const {data, error} = await dbClient
        .from('indicateur_valeur')
        .select(`${COLONNES_VALEURS.join(',')}`)
        .eq('id', valeurId);
...
export async function selectIndicateurDefinition(
    ...
    let toReturn = data ? await transformeDefinition(dbClient, data, collectiviteId, false) : null;
    return toReturn ? objectToCamel(toReturn)[0] as IndicateurDefinition : null;
}
...
export async function selectIndicateurComplet(
    ...
    let toReturn = data ? await transformeDefinition(dbClient, data, collectiviteId,true) : null;
    return toReturn? objectToCamel(toReturn)[0] as IndicateurDefinitionComplet : null;
}
...
export async function selectIndicateurChartInfo(
    ...
    const {data, error} = await dbClient
        .from('indicateur_definition')
        .select(`${COLONNES_DEFINITION_COURTE.join(',')}, groupement_id,` +
    ...
    const groupement : Groupement[] = await selectGroupementParCollectivite(dbClient, collectiviteId);
    const groupementIds = groupement.map(gp => gp.id);
    let toReturn = data?.filter((item : any)=>
        item.groupement_id === null || groupementIds.includes(item.groupement_id))
        .map(item => {
            // Récupère l'information de la confidentialité
            const plusInfo = item.plus && item.plus[0] ? item.plus[0] :
            ...
            if (item.sans_valeur && enfantsTransforme.length>0){
                ...
                if(count === total || (item.identifiant === ID_COMPACITE_FORMES_URBAINES && count >=1)){
                    // On affiche les valeurs du premier enfant rempli
                    const premierRempli = enfantsTransforme.find((enf : any) => enf.rempli);
...
export async function getValeursComparaison(
    ...
    collectiviteId : number,
    source : string
): Promise<{resultats: ValeurComparaison, objectifs: ValeurComparaison} | null>{
    const valeursUtilisateur =
        await selectIndicateurValeurs(dbClient, indicateurId, collectiviteId, null);
    const valeursSource = await selectIndicateurValeurs(dbClient, indicateurId, collectiviteId, source);
    ...
    valeursSource.map(aAppliquer => {
        ...
        const aEcraser = valeursUtilisateur?.find(d => d.annee === aAppliquer.annee);
        ...
        if (aAppliquer.resultat){
            ...
            ligne.conflit = ligne.valeurAEcraser? ligne.valeurAEcraser !== ligne.valeurAAppliquer : false;
    ...
    return {
        resultats : {lignes : lignesResultat, conflits: nbConflitsResultat, ajouts: nbAjoutsResultat},
        objectifs : {lignes : lignesObjectif, conflits: nbConflitsObjectif, ajouts: nbAjoutsObjectif},
    };
}
...
async function transformeDefinition(
    ...
    return data.map(item => {
        ...
        const plusInfo = item.plus && item.plus[0] ? item.plus[0] :
            { commentaire: null, confidentiel: false, collectiviteId : collectiviteId };
        ...
        if (complet){
            valeurs = item.valeurs?.filter((val: any) => val.source === null || val.source.length === 0)|| [];
            valeurs = dateEnAnnee(valeurs, true);
            services = item.services ? item.services : [];
            pilotes = item.pilotes
                .map((p : any) => {
                    return {
                        tagId : undefined,
                        userId : undefined,
            ...
            fiches = item.fiches;
            fiches_non_classees = fiches.filter((fiche : any) => fiche.plans === null || fiche.plans.length === 0);
        }
        ...
        item.categories.forEach((cat : any) => {
            if (cat.collectivite_id !== null) {
                if (complet && cat.collectivite_id === collectiviteId) {
                    ...
                    categoriesUtilisateur.push({id: cat.id, nom: cat.nom, collectivite_id: cat.collectivite_id});
                }
            }else if(cat.groupement_id !== null){
                const groupe = groupement.filter((g: any)=> g.id === cat.groupement_id);
                if(groupe.length>0 && groupe[0].collectivites && groupe[0].collectivites.includes(collectiviteId)){
        ...
        return {
            ...
            services : services,
            fiches : fiches,
            fiches_non_classees : fiches_non_classees // Ajoute le champ 'est_perso'
        };
    });
}
