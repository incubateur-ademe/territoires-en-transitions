 ...
 * @return ?
 */
export const saveData = async (supabaseClient: TSupabaseClient, memoire : TMemoire, fiches : TFicheActionImport[]) => {
    ...
    for(fiche of fiches){
        ...
        // Sauvegarde axe
        const axe : Database['public']['Tables']['axe']['Insert'] = await saveAxes(supabaseClient, memoire, axeImport);
        ficheToSave.axes = axe==null?null:[axe];
        // Sauvegarde fiche_action
        if(ficheToSave.titre!=null){
            const { error, data } = await supabaseClient.from('fiches_action').upsert(ficheToSave);
            if (error) {
