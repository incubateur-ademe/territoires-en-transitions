import { Views } from "../_shared/typeUtils.ts";
...
export type TFinanceur = Omit<Database["public"]["CompositeTypes"]["financeur_montant"], 'id' | 'montant_ttc' | 'financeur_tag'> & {
    id?: number;
    montant_ttc?: number;
    financeur_tag: TTag;
};
export type TIndicateur = Omit<Database["public"]["CompositeTypes"]["indicateur_generique"], 'indicateur_id' | 'indicateur_personnalise_id'> & {
...
export type TPersonne = Omit<Database["public"]["CompositeTypes"]["personne"], 'tag_id' | 'user_id'> & {
    tag_id?: number | null;
    user_id?: string | null;
};
export type TTag = {
...
export type TFicheResume = Omit<Database["public"]["Views"]["fiche_resume"]["Row"], 'plans'> & {
    plans: Database["public"]["Tables"]["axe"]["Insert"][] | [null] | null;
};
export type TFicheAction = Omit<Views<"fiches_action">, 'thematiques' | 'sous_thematiques' | 'partenaires' | 'structures' | 'pilotes' | 'referents' | 'annexes' | 'axes' | 'actions' | 'indicateurs' | 'services' | 'financeurs' | 'fiches_liees'> & {
