...
from api.models.tortoise.indicateur_referentiel_commentaire import (
    IndicateurReferentielCommentaire_Pydantic,
...
@router.get(
    "/{epci_id}/all", response_model=List[IndicateurReferentielCommentaire_Pydantic]
)
...
async def get_indicateur_referentiel_commentaire(epci_id: str, indicateur_id: str):
    query = IndicateurReferentielCommentaire.get(
        epci_id=epci_id, indicateur_id=indicateur_id, latest=True
    )
    try:
        return await IndicateurReferentielCommentaire_Pydantic.from_queryset_single(
            query
        )
