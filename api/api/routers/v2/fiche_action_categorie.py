...
from api.models.tortoise.fiche_action_categorie import (
    FicheActionCategorie_Pydantic,
...
@router.get(
    "/{epci_id}/{uid}",
    response_model=FicheActionCategorie_Pydantic,
    responses={404: {"model": HTTPNotFoundError}},
)
async def get_fiche_action_categorie(epci_id: str, uid: str):
    query = FicheActionCategorie.get(
        epci_id=epci_id, uid=uid, latest=True, deleted=False
    )
    try:
        return await FicheActionCategorie_Pydantic.from_queryset_single(query)
    except DoesNotExist as error:
        raise HTTPException(
            status_code=404, detail=f"fiche_action_categorie {epci_id}/{uid} not found"
        )
