...
-- create default categories in old categories
...
-- create plans from categories
truncate table "planaction";
...
-- search orphaned fiches
(
    select epci.nom        collectivite,
           ficheaction.uid fiche_uid
    from ficheaction
             join epci on ficheaction.epci_id = epci.uid
)
except
(
    select epci.nom                                                 collectivite,
           jsonb_array_elements(fiches_by_category) ->> 'fiche_uid' fiche_uid
    from planaction
             join epci on planaction.epci_id = epci.uid
);
