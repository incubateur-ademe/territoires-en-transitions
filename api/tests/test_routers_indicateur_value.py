...
def test_update_indicateur_value(client: TestClient):
    ...
    response = client.post(post_path, json=existing_indicateur_value, headers=auth_headers())
...
def test_create_mismatched_indicateur_value(client: TestClient):
    ...
    post_path = f"{path}/{indicateur_value['epci_id']}"
    response = client.post(post_path, json=mismatched_indicateur_value, headers=auth_headers())
