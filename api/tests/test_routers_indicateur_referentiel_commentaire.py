...
path = "/v1/indicateur_referentiel_commentaire"
...
def test_crud_item(client: TestClient, event_loop: asyncio.AbstractEventLoop):
    # POST /v1/indicateur_referentiel_commentaire/epci_id
    response = client.post(post_path, json=indicateur_referentiel_commentaire)
    assert response.status_code == 200
    assert response.json()['indicateur_id'] == indicateur_referentiel_commentaire['indicateur_id']

    # GET /v1/indicateur_referentiel_commentaire/epci_id/all
    response = client.get(list_path)
    assert response.status_code == 200
    assert len(response.json()) == 1
    assert response.json()[0]['indicateur_id'] == indicateur_referentiel_commentaire['indicateur_id']

    # GET /v1/indicateur_referentiel_commentaire/epci_id/indicateur_id
    response = client.get(item_path)
    assert response.status_code == 200
    assert response.json()['indicateur_id'] == indicateur_referentiel_commentaire['indicateur_id']

    # DELETE /v1/indicateur_referentiel_commentaire/epci_id/indicateur_id
...
def test_update_indicateur_referentiel_commentaire(client: TestClient):
    ...
    response = client.post(post_path, json=existing_indicateur_referentiel_commentaire)
...
def test_create_mismatched_indicateur_referentiel_commentaire(client: TestClient):
    ...
    post_path = f"{path}/{indicateur_referentiel_commentaire['epci_id']}"
    response = client.post(post_path, json=mismatched_indicateur_referentiel_commentaire)
