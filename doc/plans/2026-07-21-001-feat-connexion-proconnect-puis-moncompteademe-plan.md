...
(`sso-moncompteademe`) pour un déploiement progressif et
réversible.
...
- **Contraintes structurantes** :
  ...
  - **Différence de claims vérifiée** (résout OQ4) : **ProConnect n'émet pas
    ...
    politique `isEmailVerified` (décision 9).
...
- **R14** *(nouveau)* : quand un compte TeT existe pour l'email mais que le
  provider ne l'atteste pas vérifié (`isEmailVerified` faux), on **ne rattache
...
- **R15** *(nouveau)* : **toute** la couche connexion unifiée (traitement
  ...
  feature flag PostHog** `sso-moncompteademe`. Flag OFF ⇒ aucun
...
3. **Table d'identités multi-provider** `utilisateur.identite_oidc` — PK
   ...
   `public.utilisateur_identite_oidc_invitation` (token hashé sha256, 24 h, index partiel
...
5. **Liaison assistée par re-connexion** : la branche « oui » de la dialog
   ...
   `utilisateur_identite_oidc_invitation` ne sert que le fallback « mot de passe oublié »
...
8. **Migration pilotée par date + phase (côté serveur) + feature flag**
   *(remplace `AUTH_LOGIN_MODE`)* :
   ...
   - Feature flag PostHog `sso-moncompteademe` (hook partagé
...
9. **Politique `isEmailVerified(provider, claims)`** : `email_verified`
...
- **Files** : migrations Sqitch `utilisateur/utilisateur_identite_oidc` + `utilisateur_identite_oidc_invitation` ;
  tables Drizzle dans `apps/backend/src/users/authentications/oidc/models/`.
...
- **Files** : `authentifier/authentifier-oidc.service.ts` (statuts `connexion`,
  `compte-desactive`, `email-non-verifie`, `non-reconnu` ; `isEmailVerified`),
...
- **Files** : `apps/app/app/(public)/auth/proconnect/{page,confirmer-session,
  ...
  (`@Controller('auth/proconnect')`, `GET creer-compte`) ;
...
- **Files backend** : `authentications/oidc/get-oidc-status/{service,router,service.spec}.ts` ;
...
- **Files front** : `apps/auth/components/ConnexionUnifiee/` (hook + blocs
  ...
  (`use-link-oidc-identity`, `link-oidc-identity.banner`,
  `incitation-liaison-oidc.modal`, `liaison-obligatoire.modal`) ;
...
- **Fait** : tests backend `oidc` (dont `get-oidc-status`) verts ;
...
1. `nx test backend 'oidc'` (dont `get-oidc-status`) vert ; typecheck
...
- **Réconciliation avec l'implémentation (2026-07-23)** : politique
  `isEmailVerified` (ProConnect n'émet pas `email_verified`), couche connexion
