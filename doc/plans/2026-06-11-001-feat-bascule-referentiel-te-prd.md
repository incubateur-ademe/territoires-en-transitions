...
type ReferentielPreferenceTE = ReferentielPreference & {
  populatedFromCaeEci?: { populatedAt: string; populatedBy: string }; // absent avant bascule depuis CAE/ECI
};
...
TE est toujours affiché (`display: true`, modulo feature flag). La règle ne se recalcule pas en continu : uniquement sur reset manuel super-admin ou batch `resetAllCollectivitesDisplayPreferences`. **Guard** : ne pas recalculer si `te.populatedFromCaeEci` est renseigné.

**Après bascule** (`te.populatedFromCaeEci` renseigné) :

| Référentiel | `mode` | `display` | `populatedFromCaeEci` |
|---|---|---|---|
| `te` | `write` | `true` | renseigné (`populatedAt`, `populatedBy`) |
...
4. En cas d'échec batch : ne pas lever le flag ; rollback = réexécution batch (idempotent grâce au garde `populatedFromCaeEci`).
...
- CTA disabled + message explicite si COT actif ou demande d'audit en cours ou audit en cours :
  - **COT actif** : « Un COT est actif pour votre collectivité. Vous devez clôturez l'audit final avant de réaliser la bascule. ».
  - **Demande d'audit en cours** : « Une demande d'audit est en cours sur {ref}. Attendez la fin de l'audit avant de basculer. » + lien vers la page labellisation concernée
  - **Audit en cours** : « Un audit est ouvert sur {ref}. Terminez-le avant de basculer. » + lien vers la page labellisation concernée.
...
|---|---|---|
| Transaction volumineuse (timeout DB) | Bascule partielle | **Risque accepté** (juin 2026) : benchmark snapshots — CT la plus chargée connue (1377 statuts CAE) → ~1,5 s sur instance test ≥ 2× plus petite que prod ; bascule ponctuelle (1 CT, pas de pic simultané). Mitigation : rollback total ; `populatedFromCaeEci` écrit en dernier. **Validation PR18** : e2e bascule complète sur cette CT + mesure durée merge/écritures statuts TE.
