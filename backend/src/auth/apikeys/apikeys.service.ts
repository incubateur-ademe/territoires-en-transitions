import { GenerateApiKeyRequest } from '@/backend/auth/apikeys/generate-api-key.request';
import { GenerateApiKeyResponse } from '@/backend/auth/apikeys/generate-api-key.response';
import { GenerateTokenRequest } from '@/backend/auth/apikeys/generate-token.request';
import { GenerateTokenResponse } from '@/backend/auth/apikeys/generate-token.response';
import {
  UserApiKeyInsert,
  userApiKeyTable,
} from '@/backend/auth/apikeys/user-api-keys.table';
import {
  AuthJwtPayload,
  AuthRole,
  AuthUser,
...
export class ApikeysService {
  ...
  async generateToken(
    ...
    const apiKeyInfo = await this.getApiKeyInfo(request.clientId);

    if (!apiKeyInfo.users) {
      throw new UnauthorizedException(
        `Invalid clientId: ${request.clientId} - user not found`
      );
    }
