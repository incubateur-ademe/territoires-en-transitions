...
export class ListDefinitionsService {
        ...
        and(
          ...
          or(
            ...
            input?.collectiviteId
              ? eq(
                  enfantGroupementCollectivite.collectiviteId,
                  input?.collectiviteId
                )
              : undefined
          )
  ...
  async getDefinitionsDetaillees(
    ...
    const groupementConditions: (SQLWrapper | SQL)[] = [
      isNull(indicateurDefinitionTable.groupementId),
    ];
    if (input?.collectiviteId) {
      /*groupementConditions.push(
        eq(groupementCollectiviteTable.collectiviteId, input?.collectiviteId)
      );*/
    }

    whereConditions.push(or(...groupementConditions)!);
    ...
    const definitionsRequest = this.databaseService.db
      .select({
        ...
        actions: indicateurActionIds.actions,
        openDataDisponible: indicateurOpenData.openData,
