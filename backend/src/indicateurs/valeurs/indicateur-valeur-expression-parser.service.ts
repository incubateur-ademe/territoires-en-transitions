...
const CT_VAL = createToken({ name: 'CT_VAL', pattern: /ct_val/ });

// tokens ajoutés au parser de base
const tokens = [VAL, OPT_VAL, CIBLE, LIMITE, CT_VAL];
...
class IndicateurValeurExprParser extends ExpressionParserBase {
  ...
  private ct_val = this.RULE('ct_val', () => {
    this.consumeFuncOneParam(CT_VAL);
  });
}
...
// Dans ce contexte une valeur "collectivité" est une valeur sélectionnée par la
// collectivité pour servir au calcul du score d'une action associée à un
// indicateur
type TypeValeurs = 'cible' | 'limite' | 'collectivite';
...
class IndicateurValeurExpressionVisitor extends getExpressionVisitor(
  ...
  ct_val(ctx: any) {
    const indicateurIdentifier = this.visit(ctx.identifier);
    if (!this.indicateurValeursComplementaires?.collectivite) {
      throw new Error(`Missing limite indicateur valeurs`);
    }
    return (
      this.indicateurValeursComplementaires.collectivite[
        indicateurIdentifier as string
      ] || null
    );
  }
