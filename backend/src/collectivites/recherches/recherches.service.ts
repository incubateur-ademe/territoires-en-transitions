...
export default class RecherchesService {
  ...
  private getFilteredCollectivitesQuery(filters: FiltersRequest, tab: Tab) {
    ...
    if (filters.typesCollectivite.length > 0) {
      const types = filters.regions.map((type) => `'${type}'`).join(', ');
      query = `${query}
  ...
  private getCollectivitesQuery(filters: FiltersRequest, tab: Tab): string {
    ...
    if (filters.nom && tab != tabEnum.Plan) {
      query = `${query}
    ...
    if (filters.departements.length > 0) {
      const departements = filters.regions
        .map((departement) => `'${departement}'`)
        .join(', ');
      query = `${query}
      AND ${collectiviteTable.departementCode.name} IN (${departements})`;
    }
  ...
  private getPlansQuery(filters: FiltersRequest) {
    ...
    if (filters.nom) {
      query = `${query}
      AND unaccent(${axeTable.nom.name}) ILIKE unaccent('%${filters.nom}%')`;
    }
