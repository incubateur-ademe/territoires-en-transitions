...
export default class CollectivitesService {
  ...
  getCollectiviteSousType(
    ...
    if (
      collectivite.typeId == collectiviteTypeId.EPCI &&
      typeBanatic &&
  ...
  async getCollectiviteAvecType(
    ...
    const collectiviteTest: boolean =
      collectivite.collectivite.typeId == collectiviteTypeId.Test;

    const collectiviteEPCI: boolean =
      collectivite.collectivite.typeId == collectiviteTypeId.EPCI;
    ...
    const epciByIdResult = await this.databaseService.db
      ...
      .where(
        and(
          ...
          eq(collectiviteTable.typeId, collectiviteTypeId.EPCI)
        )
      );
