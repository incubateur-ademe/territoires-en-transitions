...
export class ExportScoreService {
  ...
  // Add new constants for audit export
  ...
  async exportAuditScore(
    ...
    this.logger.log(
      `Exporting audit score for collectivite ${collectiviteId}, referentiel ${referentielId}, audit year ${auditYear}`
    );
    ...
    const [preAuditSnapshot, currentSnapshot] = await Promise.all([
      this.snapshotsService.get(
        collectiviteId,
        referentielId,
        `${SnapshotsService.PRE_AUDIT_SNAPSHOT_REF_PREFIX}${auditYear}`
      ),
    ...
    if (!auditeurs.length) {
      this.logger.warn(
        `No auditeurs found for collectivite ${collectiviteId}, referentiel ${referentielId}, year ${auditYear}`
      );
    }
