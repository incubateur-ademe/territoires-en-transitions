...
export default class ReferentielsScoringService {
  ...
  updateFromOrigineActions(
    ...
    initialScore.pointFait = roundTo(
      ratio *
        (origineActions?.reduce(
          (acc, origineAction) =>
            acc +
            (origineAction.score?.pointFait || 0) *
              (origineAction.ponderation || 1),
          0
        ) || 0),
      roundingDigits
    ...
    initialScore.pointProgramme = roundTo(
      ratio *
        (origineActions?.reduce(
          (acc, origineAction) =>
            acc +
            (origineAction.score?.pointProgramme || 0) *
              (origineAction.ponderation || 1),
          0
        ) || 0),
      roundingDigits
    ...
    initialScore.pointPasFait = roundTo(
      ratio *
        (origineActions?.reduce(
          (acc, origineAction) =>
            acc +
            (origineAction.score?.pointPasFait || 0) *
              (origineAction.ponderation || 1),
          0
        ) || 0),
      roundingDigits
    ...
    initialScore.pointNonRenseigne = origineActions?.length
      ? roundTo(
          ratio *
            (origineActions?.reduce(
              (acc, origineAction) =>
                acc +
                (origineAction.score?.pointNonRenseigne || 0) *
                  (origineAction.ponderation || 1),
              0
            ) || 0),
          roundingDigits
  ...
  async getClientScoresForCollectivite(
    ...
    // TODO: Change casing
    const scoreList = (scores[0].scores as any[]).map(
      (score) => score as ActionScoreType
    ) as ActionScoreType[];
