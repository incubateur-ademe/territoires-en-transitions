...
export class ScoreIndicatifService {
  ...
  async getValeursUtilisables(
    ...
    const result = input.actionIds
      .map((actionId) => ({
        actionId,
        indicateurs: indicateursAssocies
          .filter((ind) => ind.actionId === actionId)
          .map(({ indicateurId, identifiantReferentiel, unite, titre }) => {
            const sources = valeursGroupees.indicateurs.find(
              (ind) => ind.definition.id === indicateurId
            )?.sources;

            if (!sources) return null;

            const valeursUtilisees =
            ...
            const transformeValeur = (
              ...
              const utilisee = valeursUtiliseesParTypeScore[
                typeScore
              ]?.includes(v.id);
              const valeur = (
            ...
            return {
              ...
              sources: Object.values(sources)
                .map((s) => ({
                  ...
                  ordreAffichage: (s.source === NULL_SOURCE_ID
                    ? 0
                  ...
                  fait: s.valeurs
                    ...
                    .map((v) =>
                      transformeValeur(v, typeScoreIndicatifEnum.FAIT, s.source)
                    ),
                  programme: s.valeurs
    ...
    const scoresIndicatifs = formules
      .map(({ actionId, exprScore }) => {
        ...
        if (!valeursUtilisees?.length) {
          ...
          return { actionId, indicateurs, fait: null, programme: null };
        }
    ...
    const valeurs = await this.db
      ...
      .where(
        and(
          ...
          eq(actionScoreIndicateurValeurTable.collectiviteId, input.collectiviteId)
        )
      );
  ...
  private computeScore(
    ...
    // Vérifie que toutes les valeurs nécessaires sont présentes et construit le mapping des valeurs
    ...
    // Si une valeur attendue manque, log et retourne null
    if (
      indicateursAssocies.some(
        ({ identifiantReferentiel }) =>
          !(
            identifiantReferentiel &&
            valeurs[identifiantReferentiel] !== undefined
          )
      )
    ) {
      this.logger.log(
