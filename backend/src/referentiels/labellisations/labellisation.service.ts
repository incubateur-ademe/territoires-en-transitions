...
`;
  ...
  async getParcoursLabellisation({
    ...
    const criteresAction = actionConditionDefinitions.map(
      addIsScoreConditionSatisfied(snapshot.scores)
    );
  ...
  async getEtoileCible({
    ...
    const etoilesDefinitions = await this.getEtoileDefinitions();
...
>(actionScoresTree: A) {
  return (
    actionCondition: Awaited<
      ReturnType<LabellisationService['listActionConditionDefinitions']>
    >[number]
  ) => {
