...
def test_cannot_insert_action_commentaire_with_wrong_user_uid(
    cursor,
    supabase_client: supabase.Client,
):
...
def test_anyone_can_retrieve_action_commentaire(supabase_client, cursor):
    ...
    make_action_api_scenario.assert_anyone_can_retrieve_entity(
        insertion_sql=make_sql_insert_action_commentaire(
            user_uid=editor_uid, commentaire="lala", action_id="cae_1.2.3"
        ),
