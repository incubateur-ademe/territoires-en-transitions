...
select ok((select (add_user(99, 'no@no.no', 'admin', null) -> 'error') is not null),
          'When not associated to the collectivité add_user should return an error');

select ok((select (add_user(10, 'no@no.no', 'admin', null) -> 'error') is not null),
...
select (add_user(10, ru.email, 'edition', 'referent') ->> 'added') as added
...
select isnt_empty('select * from private_collectivite_membre pcm '
                      'join registered_user ru on ru.user_id = pcm.user_id and collectivite_id = 10 and fonction = ''referent'';',
                  'The added user should have referent fonction on the collectivité.');


--- Add a registered user that is already associated to the collectivité.
select (add_user(10, ru.email, 'admin', null) ->> 'error') as error
...
select add_user(10, rl.email, 'edition', null) ->> 'added' as added
...
select (add_user(10, 'no@no.no', 'edition', null) ->> 'invitation_id')::uuid as id
into invitation;
