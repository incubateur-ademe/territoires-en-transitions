-- Revert tet:notes_complementaires_column_deletion from pg

BEGIN;

--
-- BEFORE. Drop related functions and views before adding back the notes_complementaires column
DROP TRIGGER IF EXISTS save_history ON public.fiche_action;
DROP FUNCTION IF EXISTS historique.save_fiche_action();
DROP FUNCTION IF EXISTS public.plan_action_export(id integer);
...
-- Restore the notes_complementaires columns
alter table public.fiche_action
    add column notes_complementaires varchar(20000);

alter table historique.fiche_action
    add column notes_complementaires varchar(20000),
    add column previous_notes_complementaires varchar(20000);


-- Recreate the view with notes_complementaires (restore the state before this migration)
...
SELECT fa.modified_at,
       ...
       act.actions,
       (
       SELECT array_agg(ROW (indi.id, indi.groupement_id, indi.collectivite_id, indi.identifiant_referentiel, indi.titre, indi.titre_long, indi.description, indi.unite, indi.borne_min, indi.borne_max, indi.participation_score, indi.sans_valeur_utilisateur, indi.valeur_calcule, indi.modified_at, indi.created_at, indi.modified_by, indi.created_by, NULL::text, NULL::character varying(16), NULL::integer, NULL, NULL, NULL)::indicateur_definition) AS array_agg
       FROM (
            SELECT id.id,
                   id.groupement_id,
                   id.collectivite_id,
