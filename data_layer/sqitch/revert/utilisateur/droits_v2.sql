...
-- Table dcp
drop policy if exists allow_insert on dcp;
drop policy if exists allow_read on dcp;
drop policy if exists allow_update on dcp;
drop policy if exists allow_delete on dcp;
...
alter policy allow_insert on indicateur_resultat_commentaire with check (have_edition_acces(collectivite_id));
alter policy allow_update on indicateur_resultat_commentaire using (have_edition_acces(collectivite_id));
