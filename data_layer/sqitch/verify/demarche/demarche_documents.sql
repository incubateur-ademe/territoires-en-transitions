...
BEGIN
    ...
    -- Une pièce peut être satisfaite sans document : le CHECK hérité de
    -- preuve_base (fichier XOR lien) est remplacé par « au plus un des deux ».
    ASSERT (
        SELECT COUNT(*) = 0
        FROM pg_constraint
        WHERE conrelid = 'public.demarche_document'::regclass
    ...
    ASSERT (
        ...
        FROM pg_constraint
        WHERE conrelid = 'public.demarche_document'::regclass
          AND conname = 'demarche_document_fichier_ou_lien'
    ), 'demarche_document doit autoriser une ligne sans fichier ni lien (couverture plateforme)';
    ...
    ASSERT (
        ...
        WHERE n.nspname = 'public'
          AND c.relname IN ('demarche_document_definition',
                            ...
                            'demarche_document')
    ), 'RLS doit être activée sur les 3 tables demarche_document*';
    ...
    ASSERT (
        ...
        FROM pg_policies
        WHERE schemaname = 'public'
          AND tablename = 'demarche_document'
