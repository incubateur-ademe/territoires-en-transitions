...
$$
    ...
    BEGIN
        ...
        -- Compte les litteraux entre apostrophes : deux valeurs, pas une de plus.
        -- Sans ce test, une contrainte admettant une troisieme etape passerait.
        ASSERT (length(contrainte_etape) - length(replace(contrainte_etape, '''', ''))) / 2 = 2,
            'La contrainte ne doit admettre que deux etapes, or : ' || contrainte_etape;
        ...
        -- Les bornes sont lues sur la forme normalisee par pg_get_constraintdef.
        -- Un LIKE '%3%' laisserait passer note <= 30 ; le motif exclut donc un
        -- chiffre supplementaire apres la borne haute.
        ASSERT contrainte_note ~ 'note >= 0' AND contrainte_note ~ 'note <= 3([^0-9]|$)',
