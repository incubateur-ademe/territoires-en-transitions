...
COMMENT ON TABLE public.demarche_document_substitution IS
    'Substitution déclarative entre pièces attendues : déposer substitut_id couvre document_id. C''est ainsi que le document global du PCAET couvre toutes les sections, sans règle codée en dur.';
...
-- ===========================================================================
-- 3. Couverture déclarée sans dépôt de document.
--    Table distincte : le CHECK XOR de preuve_base interdit une ligne de
--    demarche_document sans fichier ni lien.
-- ===========================================================================
CREATE TABLE public.demarche_document_couverture (
    demarche_id integer     NOT NULL
        REFERENCES public.demarche(id) ON DELETE CASCADE,
    document_id text        NOT NULL
        REFERENCES public.demarche_document_definition(id),
    source      text        NOT NULL CHECK (source IN ('plan_actions')),
    created_at  timestamptz NOT NULL DEFAULT now(),
    created_by  uuid        NULL REFERENCES auth.users(id),
    PRIMARY KEY (demarche_id, document_id, source)
);

COMMENT ON TABLE public.demarche_document_couverture IS
    'Déclaration explicite qu''une pièce attendue est couverte sans dépôt de document (aujourd''hui : comprise dans le plan d''actions suivi sur la plateforme).';

CREATE INDEX demarche_document_couverture_document_id_idx
    ON public.demarche_document_couverture (document_id);
...
ALTER TABLE public.demarche_document_couverture ENABLE ROW LEVEL SECURITY;

-- ===========================================================================
-- 4. Modèle documentaire du PCAET : 1 document global + 9 pièces attendues.
...
VALUES
    ('document_global', 'pcaet',
     'Document global du PCAET',
     'Document unique regroupant l''ensemble des pièces attendues. Son dépôt couvre toutes les sections du dossier.',
     false, 0, 'global', NULL),
    ('diagnostic', 'pcaet', 'Diagnostic', '', true, 1, 'section', NULL),
    ('strategie_territoriale', 'pcaet', 'Stratégie territoriale', '', true, 2, 'section', NULL),
    ('plan_actions', 'pcaet', 'Plan d''actions', '', true, 3, 'section', NULL),
    ('dispositif_suivi_evaluation', 'pcaet', 'Dispositif de suivi et d''évaluation', '',
     true, 4, 'section', 'plan_actions'),
    ('ees', 'pcaet', 'EES (évaluation environnementale stratégique)', '', false, 5, 'section', NULL),
    ('deliberation_adoption', 'pcaet', 'Délibération d''adoption du PCAET', '', false, 6, 'section', NULL),
    ('memoire_reponse_avis', 'pcaet', 'Mémoire de réponse aux avis institutionnels', '',
     false, 7, 'section', NULL),
    ('synthese_consultation_publique', 'pcaet',
     'Synthèse des contributions et réponses à la consultation publique', '',
     false, 8, 'section', NULL),
    ('bilan_pcaet_precedent', 'pcaet', 'Bilan du PCAET précédent', '', false, 9, 'section', NULL)
...
SELECT definition.id, 'document_global'
