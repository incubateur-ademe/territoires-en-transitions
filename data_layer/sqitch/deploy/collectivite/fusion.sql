-- Deploy tet:collectivite/fusion to pg
...
-- Création de la table collectivite_banatic_type pour remplacer l'enum nature
-- et ainsi ajouter la signification des acronymes pour notre compréhension
create table collectivite_banatic_type
(
  id   text primary key,
  nom  text                                            not null,
  type text check (type IN ('EPCI à fiscalité propre', 'Syndicat mixte', 'Syndicat de communes',
                            'Autre type de syndicat')) not null
);
comment on table collectivite_banatic_type is 'https://www.banatic.interieur.gouv.fr/';
comment on column collectivite_banatic_type.id is 'Acronyme de la nature';
comment on column collectivite_banatic_type.nom is 'Nom complet de la nature';
comment on column collectivite_banatic_type.type is 'Regroupement des natures';

insert into collectivite_banatic_type (id, nom, type)
VALUES
  -- EPCI à fiscalité propre
  ('METRO', 'Métropole', 'EPCI à fiscalité propre'),
  ('CU', 'Communauté urbaine', 'EPCI à fiscalité propre'),
  ('CA', 'Communauté d''agglomération', 'EPCI à fiscalité propre'),
  ('CC', 'Communauté de communes', 'EPCI à fiscalité propre'),
  -- Syndicat mixte
  ('SMF', 'Syndicat mixte fermé', 'Syndicat mixte'),
  ('SMO', 'Syndicat mixte ouvert', 'Syndicat mixte'),
  -- Syndicat de communes
  ('SIVU', 'Syndicat à vocation unique', 'Syndicat de communes'),
  ('SIVOM', 'Syndicat à vocation multiple', 'Syndicat de communes'),
  -- Autre type de syndicat
  ('POLEM', 'Pôle métropolitain', 'Autre type de syndicat'),
  ('PETR', 'Pôle d''équilibre territorial et rural', 'Autre type de syndicat'),
  ('EPT', 'Établissement public territorial', 'Autre type de syndicat');

-- Ajout de nouvelles colonnes à la table collectivité
-- pour ajouter les régions et départements
-- ainsi que regrouper epci, commune et collectivite_test
alter table collectivite
  add column nom              text,
  add column type             text check (type in ('epci', 'commune', 'region', 'departement', 'test')),
  add column commune_code     codegeo,
  add column siren            siren,
  add column departement_code varchar(3),
  add column region_code      varchar(2),
  add column nature_insee     text references collectivite_banatic_type,
  add column population       integer;

-------------------------------
-- REMPLISSAGE COLLECTIVITES --
-------------------------------

-- Ajoute les données des EPCI
update collectivite
set nom          = epci.nom,
    siren        = epci.siren,
    nature_insee = epci.nature :: text,
    type         = 'epci'
from (select * from epci) as epci
where collectivite.id = epci.collectivite_id;

-- Ajoute les données des communes
update collectivite
set nom          = commune.nom,
    commune_code = commune.code,
    type         = 'commune'
from (select * from commune) as commune
where collectivite.id = commune.collectivite_id;

-- Ajoute les données des collectivités test
update collectivite
set nom  = test.nom,
    type = 'test'
from (select * from collectivite_test) as test
where collectivite.id = test.collectivite_id;

-- Ajoute les données de import.banatic
update collectivite
...
-- Ajoute les données de import.commune
update collectivite
set departement_code = i.departement_code,
    region_code      = i.region_code,
    population       = i.population
...
-- Modifie le type des départements anciennement placé dans la table epci
update collectivite
set nature_insee = null,
    type         = 'departement'
where unaccent(nom) ilike 'Departement%';

-- Retrouve le code du département.
update collectivite
set departement_code = i.code,
    region_code      = i.region_code,
    population       = i.population
from (select * from imports.departement) as i
where type = 'departement'
  AND ((nom = 'Departement de la Réunion' and i.libelle = 'La Réunion') or
       trim(unaccent(regexp_replace(regexp_replace(nom, '^(Département|Departement)\s*(du|de la|de l)?\s*', '', 'i'), '[-'']', ' ', 'g'))) =
       unaccent(regexp_replace(i.libelle, '[-'']', ' ', 'g')));

-- Modifie le type des régions anciennement placé dans la table epci
update collectivite
set nature_insee = null,
    type         = 'region'
where nom = 'Grand Est' or unaccent(nom) ilike 'Region%';

-- Retrouve le code de la région.
update collectivite
set region_code = i.code,
    population  = i.population
from (select * from imports.region) as i
where type = 'region'
  and trim(unaccent(regexp_replace(nom, '^(Région|Region)\s+', '', 'i'))) = unaccent(i.libelle);

-- Ajoute les régions manquantes
insert into collectivite (nom, type, region_code, population)
select concat('Région ',i.libelle), 'region', i.code, i.population
from imports.region i
where i.code not in (select region_code from collectivite where type = 'region');

-- Ajoute les départements manquants
insert into collectivite (nom, type, departement_code, region_code, population)
select concat('Département ',i.libelle), 'departement', i.code, i.region_code, i.population
from imports.departement i
where i.code not in (select departement_code from collectivite where type = 'departement');

-- Met à jour les sirens des départements
UPDATE collectivite
SET siren = s.siren
FROM (VALUES
          ('229740014','974'),
...
WHERE collectivite.departement_code = s.code
and collectivite.type = 'departement';

-- Met à jour les sirens des régions
UPDATE collectivite
SET siren = s.siren
FROM (VALUES
          ('232500017','43'),
...
WHERE collectivite.region_code = s.code
  and collectivite.type = 'region';

-- Gère les collectivités présente dans la table collectivite mais pas dans les sous tables
update collectivite
set type = 'test',
    nom = concat(coalesce(collectivite.nom, ''), '-sanstype')
where type is null;

-- Déplace le trigger qui créé les compétences banatic d'epci à collectivite
...
SELECT collectivite.id                          AS collectivite_id,
       collectivite.nom::character varying(300) AS nom
FROM collectivite
ORDER BY unaccent(collectivite.nom);
...
alter table question
  alter column types_collectivites_concernees
    set data type text[]
    using array [types_collectivites_concernees]::text[];
drop type type_collectivite;
drop function collectivite_card(axe);
drop trigger if exists axe_insert_automatisation on axe;
drop trigger if exists fiche_action_insert_automatisation on fiche_action;
drop trigger if exists dcp_upsert_automatisation on dcp;
drop trigger if exists dcp_insert_automatisation on dcp;
drop trigger if exists collectivite_membre_upsert_automatisation on private_utilisateur_droit;
drop trigger if exists collectivite_membre_upsert_automatisation on private_collectivite_membre;
drop trigger if exists utilisateur_droit_upsert_automatisation on private_utilisateur_droit;
drop trigger if exists utilisateur_droit_delete_automatisation on private_utilisateur_droit;
drop function if exists automatisation.send_collectivite_plan_action_json_n8n();
drop function if exists automatisation.send_delete_collectivite_membre_json_n8n();
drop function if exists automatisation.send_insert_users_json_n8n();
drop function if exists automatisation.send_upsert_collectivite_membre_json_n8n();
drop function if exists automatisation.send_upsert_collectivites_json_n8n();
drop function if exists automatisation.send_upsert_users_json_n8n();
drop table if exists automatisation.automatisation_uri;
drop type if exists automatisation.automatisation_type;
drop view if exists automatisation.collectivite_membre_crm;
drop view if exists automatisation.collectivites_crm;
drop view if exists automatisation.collectivite_plan_action;
...
-- materialized view collectivite_card MODIFIE
create materialized view collectivite_card as
WITH referentiel_score AS
       ...
       (SELECT c.id                                                as collectivite_id,
               ...
               CASE
                 WHEN c.type = 'epci' THEN
                   (CASE
                      WHEN c.nature_insee like 'S%' THEN 'syndicat'
                      ELSE c.nature_insee
                     END)
                 ELSE c.type
                 END                                               AS type_collectivite,
               COALESCE(c.commune_code::character varying, c.siren::character varying,
                        ''::character varying)                     AS code_siren_insee,
               COALESCE(c.region_code, ''::character varying)      AS region_code,
               COALESCE(c.departement_code, ''::character varying) AS departement_code,
               ...
               s.completude_max
        FROM collectivite c
               LEFT JOIN labellisation l ON l.collectivite_id = c.id
               LEFT JOIN referentiel_score s ON s.collectivite_id = c.id
        WHERE c.type != 'test')
...
-- type type_collectivite SUPPRIME

-- function private.collectivite_type(integer) MODIFIE
create function private.collectivite_type(collectivite_id integer) returns text[]
  language sql
BEGIN
  ATOMIC
  SELECT (
           case
             when c.type not in ('epci', 'test') then concat('{"', c.type, '"}')::text
             when t.type in ('Syndicat mixte', 'Syndicat de communes') then '{"EPCI", "syndicat"}'::text
             else '{"EPCI"}'::text
             end)::text[] AS type_collectivite
  FROM collectivite c
...
-- view collectivite_identite MODIFIE
create view collectivite_identite(id, population, type, localisation) as
SELECT c.id,
       COALESCE(private.population_buckets(c.population), '{}'::text[]) AS population,
       COALESCE(private.collectivite_type(c.id), '{}'::text[])          AS type,
       CASE
         WHEN ir.drom THEN '{"DOM"}'::text
         ELSE '{"Metropole"}'::text
         END ::text[]                                                   AS localisation
FROM collectivite c
...
BEGIN
  ATOMIC
  SELECT jsonb_build_object('population', ci.population, 'type', ci.type, 'localisation',
                            ci.localisation) AS jsonb_build_object
  ...
  WITH la AS (SELECT audit.id,
       ...
       evaluation_payload AS (SELECT transaction_timestamp()                     AS "timestamp",
                              ...
                              FROM (la
                                JOIN LATERAL labellisation.audit_evaluation_payload(
                                  ROW (la.id, la.collectivite_id, la.referentiel, la.demande_id, la.date_debut, la.date_fin, la.valide, la.date_cnl, la.valide_labellisation, la.clos),
                                  audit_personnalisation_payload.pre_audit, ((la.sujet IS NOT NULL) AND (la.sujet = ANY
                                                                                                         (ARRAY ['labellisation'::labellisation.sujet_demande, 'labellisation_cot'::labellisation.sujet_demande])))) ep(referentiel, statuts, consequences)
                                    ON (true))),
       personnalisation_payload AS (SELECT transaction_timestamp()                                                                AS "timestamp",
                                           ...
                                           jsonb_build_object('identite',
                                                              ...
                                                              'reponses',
                                                              (SELECT labellisation.json_reponses_at(la.collectivite_id,
                                                                                                     CASE
                                                                                                       WHEN audit_personnalisation_payload.pre_audit
                                                                                                         THEN la.date_debut
                                                                                                       WHEN (
                                                                                                         (la.sujet IS NOT NULL) AND
                                                                                                         (la.sujet = ANY
                                                                                                          (ARRAY ['labellisation'::labellisation.sujet_demande, 'labellisation_cot'::labellisation.sujet_demande])))
                                                                                                         THEN la.date_cnl
                                                                                                       ELSE la.date_fin
...
create function labellisation.evaluate_audit_statuts(audit_id integer, pre_audit boolean,
...
BEGIN
  ...
  FROM ((evaluation.current_service_configuration() conf(evaluation_endpoint, personnalisation_endpoint, created_at)
    JOIN labellisation.audit_personnalisation_payload(evaluate_audit_statuts.audit_id, evaluate_audit_statuts.pre_audit,
...
-- view collectivite_carte_identite MODIFIE
...
SELECT c.id                                        as collectivite_id,
       ...
       CASE
         WHEN c.type = 'epci' then
           case
             when t.type in ('Syndicat mixte', 'Syndicat de communes') then 'syndicat'
             else 'EPCI'
             end
         ELSE c.type
         END                                       AS type_collectivite,
       COALESCE(c.commune_code::character varying, c.siren::character varying,
                ''::character varying)             AS code_siren_insee,
       COALESCE(ir.libelle, ''::character varying) AS region_name,
       COALESCE(id.libelle, ''::character varying) AS departement_name,
       case
         when c.type = 'epci' then 'BANATIC 2023'
         when c.type = 'commune' then 'Insee Populations légales 2020 parues 29/12/2022'
         else ''
         end                                       AS population_source,
       COALESCE(c.population, 0)                   AS population_totale,
       COALESCE(cot.actif, false)                  AS is_cot
FROM collectivite c
       LEFT JOIN collectivite_banatic_type t ON c.nature_insee = t.id
       LEFT JOIN imports.region ir ON c.region_code::text = ir.code::text
       LEFT JOIN imports.departement id ON c.departement_code::text = id.code::text
       LEFT JOIN cot ON cot.collectivite_id = c.id;
...
-- materialized view stats.collectivite MODIFIE
create materialized view stats.collectivite as
WITH info AS (SELECT c.id                                  as collectivite_id,
                     c.nom,
                     CASE
                       WHEN c.type = 'epci' then
                         case
                       ...
                       ELSE c.type
                       END                                 AS type_collectivite,
                     COALESCE(c.nature_insee, c.type)      AS nature_collectivite,
                     COALESCE(c.commune_code, c.siren, '') AS code_siren_insee,
                     COALESCE(ir.libelle, '')              AS region_name,
                     COALESCE(ir.code, '')                 AS region_code,
                     COALESCE(id.libelle,
                              '')                          AS departement_name,
                     COALESCE(id.code,
                              '')                          AS departement_code,
...
-- materialized view stats.carte_epci_par_departement MODIFIE
...
BEGIN
  ...
  SELECT v.user_id,
         ...
         md5(((('visite'::text || v.page) || (v.user_id)::text) ||
              (v."time")::text))                                                                                                        AS insert_id,
         jsonb_build_object('page', v.page, 'tag', v.tag, 'onglet', v.onglet, 'collectivite_id', v.collectivite_id,
                            'niveau_acces', pud.niveau_acces, 'fonction', pcm.fonction, 'champ_intervention',
                            pcm.champ_intervention, 'collectivite',
                            to_json(c.*))                                                                                               AS event_properties,
         jsonb_build_object('fonctions', (SELECT array_agg(DISTINCT m.fonction) AS array_agg
                                          FROM (private_collectivite_membre m
                                            JOIN private_utilisateur_droit pud_1 ON (((m.user_id = pud_1.user_id) AND
                                                                                      (m.collectivite_id = pud_1.collectivite_id))))
                                          WHERE ((m.user_id = v.user_id) AND (m.fonction IS NOT NULL) AND pud_1.active)),
                            'auditeur', (v.user_id IN (SELECT auditeurs.user_id
...
-- function geojson(site_labellisation) MODIFIE
...
BEGIN
  ATOMIC
  SELECT COALESCE(cg.geojson, eg.geojson) AS "coalesce"
  FROM collectivite c
...
BEGIN
  ...
  FROM (SELECT iri.date_valeur,
        ...
        WHERE ((iri.collectivite_id = ($1).collectivite_id) AND (iri.metadonnee_id IS NOT NULL) AND
               (iri.resultat IS NOT NULL) AND (src.id = 'citepa'::text) AND (id.identifiant_referentiel = ANY
                                                                             (ARRAY [('cae_1.g'::character varying)::text, ('cae_1.f'::character varying)::text, ('cae_1.h'::character varying)::text, ('cae_1.j'::character varying)::text, ('cae_1.i'::character varying)::text, ('cae_1.c'::character varying)::text, ('cae_1.e'::character varying)::text, ('cae_1.d'::character varying)::text, ('cae_1.a'::character varying)::text])))) d;
END;
...
SELECT m.first_day                              AS mois,
       ...
       (SELECT count(*) FILTER (WHERE cu.type_collectivite = 'EPCI') AS count
        FROM stats.collectivite_utilisateur cu
...
-- materialized view stats.collectivite_actives_et_total_par_type MODIFIE
create materialized view stats.collectivite_actives_et_total_par_type as
WITH collectivite AS (SELECT c.collectivite_id,
                             CASE
                               WHEN t.type = 'EPCI à fiscalité propre' THEN 'EPCI'::text
                               WHEN c.type_collectivite = 'syndicat' THEN 'syndicat'::text
                               WHEN c.type_collectivite = 'commune' THEN 'commune'::text
                               ELSE NULL::text
                               END AS typologie
                      FROM stats.collectivite c
                             LEFT JOIN collectivite_banatic_type t on c.nature_collectivite = t.id)
...
-- materialized view stats.locales_collectivite_actives_et_total_par_type MODIFIE
...
-- materialized view stats.locales_evolution_total_activation MODIFIE
