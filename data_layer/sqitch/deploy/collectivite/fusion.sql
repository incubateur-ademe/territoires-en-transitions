...
-- Création de la table collectivite_type
-- pour typer les collectivités
-- et ainsi se passer des tables epci, commune et collectivite_test
create table collectivite_type
(
  id  text primary key,
  nom text not null
);

insert into collectivite_type (id, nom)
VALUES ('epci', 'EPCI'),
       ('commune', 'Commune'),
       ('region', 'Région'),
       ('departement', 'Département'),
       ('test', 'Collectivité test');
...
alter table collectivite
  add column nom              text,
  add column type_id          text references collectivite_type,
...
where type_id = 'departement'
  and trim(unaccent(regexp_replace(nom, '^(Département|Departement)\s+', '', 'i'))) = unaccent(i.libelle);

-- Modifie le type des régions anciennement placé dans la table epci
update collectivite
set nature_insee = null,
    type_id      = 'region'
where unaccent(nom) ilike 'Region%';
...
BEGIN
  ...
  SELECT (
           case
             when c.type_id not in ('epci', 'test') then concat('{"', c.type_id, '"}')::text
...
SELECT c.id                                        as collectivite_id,
       ...
       case
         when c.type_id = 'epci' then 'BANATIC 2023'
         when c.type_id = 'commune' then 'Insee Populations légales 2020 parues 29/12/2022'
         else ''
...
WITH info AS (SELECT c.id                                  as collectivite_id,
                     ...
                     CASE
                       ...
                       ELSE c.type_id
                       END                                 AS type_collectivite,
                     COALESCE(c.nature_insee, c.type_id)   AS nature_collectivite,
