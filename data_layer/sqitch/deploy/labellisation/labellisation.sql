...
-- Met à jour la table public.labellisation après une labellisation
...
-- ajoute le trigger pour mettre à jour la table après une labellisation validée
create trigger update_labellisation_before_audit_update
    before update
    on labellisation.audit
    for each row
execute procedure labellisation.update_labellisation();
