...
CREATE TABLE if not exists auth.user_api_key (
...
comment on table auth.user_api_key is 'Les api keys associées aux utilisateurs.';

create index user_api_key_client_id_index on auth.user_api_key(client_id);
