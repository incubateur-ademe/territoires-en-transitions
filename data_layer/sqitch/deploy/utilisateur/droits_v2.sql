...
-- TABLES
-- Ajoute have_lecture_acces(collectivite_id)
...
-- Change est_verifie en is_authenticated
...
SELECT
    CASE
        WHEN ( SELECT collectivite.access_restreint
               ...
               LIMIT 1)
            THEN (have_lecture_acces(collectivite_id) OR est_support() OR private.est_auditeur(collectivite_id))
        ELSE (est_verifie() OR have_lecture_acces(collectivite_id))
        END AS est_verifie;
END;
...
-- Passe la fonction en security definer
...
WHERE qtd.referentiels IS NOT NULL
  AND (est_verifie() or have_lecture_acces(c.id))
ORDER BY (
             CASE
