...
export default class ImportPersonnalisationQuestionService extends BaseSpreadsheetImporterService {
   ...
   * Similar to populateSpreadsheetWithDatabase in import-preuve-reglementaire-definition.service.ts
   */
  async populateSpreadsheetWithDatabase() {
    ...
    this.logger.log(`Found ${questions.length} questions in database`);

    // Get all choices
    const choix = await this.databaseService.db
      .select()
      .from(questionChoixTable);

    this.logger.log(`Found ${choix.length} choices in database`);

    // Prepare questions data for spreadsheet
    ...
    const questionsRowData: string[][] = questions.map((question) => {
      const record = {
        ...question,
        // Convert array to comma-separated string for spreadsheet
        typesCollectivitesConcernees:
          question.typesCollectivitesConcernees?.join(', ') || '',
      };
      return this.sheetService.getRecordRowToWrite(record, questionsHeader);
    });

    // Add header as first row
    questionsRowData.unshift(questionsHeader);

    // Prepare choices data for spreadsheet
    const choixHeader: string[] = [
      'id',
      'questionId',
      'ordonnancement',
      'formulation',
    ];
    const choixRowData: string[][] = choix.map((choice) =>
      this.sheetService.getRecordRowToWrite(choice, choixHeader)
    );

    // Add header as first row
    choixRowData.unshift(choixHeader);
    ...
    this.logger.log(
      `Successfully populated spreadsheet with ${questions.length} questions and ${choix.length} choices`
    );
  }
}
