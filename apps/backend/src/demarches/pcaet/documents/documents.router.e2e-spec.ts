...
describe('Documents d’une démarche PCAET', () => {
  ...
  test('Le modèle de démarche est servi par la base, dossier vide', async () => {
    ...
    expect(snapshot.couvertures).toEqual([]);
    expect(snapshot.planActionRattache).toBe(false);
    ...
    expect(
      sections.find((section) => section.id === 'dispositif_suivi_evaluation')
  ...
  test('La couverture par le plan d’actions exige un plan rattaché et une pièce éligible', async () => {
    const { caller, collectivite, demarche } = await freshDemarche();

    // Sans plan rattaché, la déclaration est refusée.
    await expect(
    ...
    ).rejects.toThrow('Aucun plan d’actions n’est rattaché à la démarche');

    // Une pièce que le modèle ne rend pas couvrable est refusée aussi.
    await expect(
      caller.demarches.pcaet.documents.setCouverture({
        collectiviteId: collectivite.id,
        demarcheId: demarche.id,
    ...
    expect(snapshot.planActionRattache).toBe(true);
    expect(snapshot.couvertures).toEqual([
    ...
    expect(apresRetrait.couvertures).toEqual([]);
  });
