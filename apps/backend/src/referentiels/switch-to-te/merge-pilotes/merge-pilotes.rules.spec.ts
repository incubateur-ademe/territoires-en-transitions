...
describe('dedupePilotes', () => {
  ...
  it('déduplique par userId', () => {
    expect(
      dedupePilotes([userPilote('user-1'), userPilote('user-1')])
    ).toEqual([userPilote('user-1')]);
  });
  ...
  it('conserve userId et tagId distincts comme deux lignes', () => {
    expect(
      dedupePilotes([userPilote('user-1'), tagPilote(1)])
    ).toEqual([userPilote('user-1'), tagPilote(1)]);
  });
});
...
describe('mergePilotesForCible', () => {
  ...
  it('laisse inchangé une origine déjà au niveau mesure', () => {
    expect(
      merge([createOrigine(ReferentielIdEnum.CAE, 'cae_6.1.3')])
    ).toEqual([userPilote('user-cae')]);
  });
...
describe('toActionPiloteCreate', () => {
  it('mappe collectiviteId, actionId, userId et tagId avec null par défaut', () => {
    expect(toActionPiloteCreate(42, 'te_1.1.1', userPilote('user-1'))).toEqual({
      collectiviteId: 42,
      actionId: 'te_1.1.1',
      userId: 'user-1',
      tagId: null,
    });
    expect(toActionPiloteCreate(42, 'te_1.1.1', tagPilote(7))).toEqual({
      collectiviteId: 42,
      actionId: 'te_1.1.1',
      userId: null,
      tagId: 7,
