...
import { ListPreuveLabellisationInput } from './list-preuve.input';

@Injectable()
export class ListPreuveService {
  private readonly logger = new Logger(ListPreuveService.name);
  ...
  async listPreuvesLabellisation(
    { demandeId }: ListPreuveLabellisationInput,
  ...
  > {
    ...
    try {
      ...
      const preuves = await this.databaseService.db
        ...
        .leftJoin(
          dcpTable,
          eq(preuveLabellisationTable.modifiedBy, dcpTable.userId)
        )
