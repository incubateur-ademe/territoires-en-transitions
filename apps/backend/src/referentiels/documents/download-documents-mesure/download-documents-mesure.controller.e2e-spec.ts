...
describe('DownloadDocumentsMesureController', () => {
  ...
  test("laisse hors de l'archive un document confidentiel pour un non membre", async () => {
    const { collectiviteId, membreCaller, uploadFichier } =
      await createCollectivite();

    const publicFichier = await uploadFichier({ fileName: 'public.pdf' });
    const confidentielFichier = await uploadFichier({
    ...
    await membreCaller.referentiels.actions.addPreuveReglementaire({
      collectiviteId,
      preuveId: MESURE.attendu,
      fichierId: publicFichier.id,
    });
    await membreCaller.referentiels.actions.addPreuveReglementaire({
      collectiviteId,
      preuveId: MESURE.attendu,
      fichierId: confidentielFichier.id,
    });
    ...
    const entries = await listArchiveEntries(response.body);
    expect(entries).toEqual(['public.pdf']);
  });
