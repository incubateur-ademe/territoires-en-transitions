...
import { indicateurListItemSchema } from './list-indicateurs.output';

import { addCollectiviteToGroupement } from '../../../collectivites/shared/models/groupement.test-fixture';

type Input = inferProcedureInput<
  AppRouter['indicateurs']['indicateurs']['list']
>;
...
describe('ListIndicateursRouter', () => {
  ...
  describe('structure et règles métier', () => {
    test(`vérifie la structure du résultat`, async () => {
      ...
      const parsedResult = z.array(indicateurListItemSchema).safeParse(result);
  ...
  describe('avec filtres', () => {
    test(`filtre par identifiantReferentiel`, async () => {
      ...
      const toCheck1 = indicateurListItemSchema.safeParse(result[0]);
    ...
    test('filtre par estRempli (user values)', async () => {
      ...
      const { data: remplis } = await caller.indicateurs.indicateurs.list({
        collectiviteId: 1,
      ...
      const { data: nonRemplis } = await caller.indicateurs.indicateurs.list({
        collectiviteId: 1,
    ...
    test('filtre par ficheIds', async () => {
      ...
      const { data } = await caller.indicateurs.indicateurs.list(input);
    ...
    test("filtre par ficheIds - le parent remonté contient le bon nombre d'enfants", async () => {
      ...
      const parentWithoutGroupement = indicateursWithoutGroupement.find(
        (i) => i.id === parent.id
      );
      if (!parentWithoutGroupement) {
        expect.fail('Parent not found');
      }

      // Vérifier que le tableau enfants du parent ne contient pas les enfants des groupements
      expect(parentWithoutGroupement.enfants.length).toBe(0);

      // Maintenant on ajoute la collectivite 1 au groupement et
      // on vérifie que le parent remonte avec les enfants du groupement

      await addCollectiviteToGroupement({
        database,
        groupementId: groupement1.id,
        collectiviteId: 1,
      });
      ...
      const parentWithGroupement = indicateursWithGroupement.find(
        (i) => i.id === parent.id
      );
      if (!parentWithGroupement) {
        expect.fail('Parent not found');
      }

      // Vérifier que le tableau enfants du parent contient les enfants du groupement
      // et qu'il n'y a pas de doublons
      expect(parentWithGroupement.enfants).toContainEqual(
        expect.objectContaining({ id: enfant1.id })
      );
      expect(parentWithGroupement.enfants.length).toBe(1);
    });
    ...
    test('filtre par mesureId', async () => {
      ...
      const { data: resultEmpty } = await caller.indicateurs.indicateurs.list({
        collectiviteId: 1,
        filters: {
    ...
    test('filtre par estPerso', async () => {
      ...
      const { data: resultSansPerso } =
        await caller.indicateurs.indicateurs.list(sansPerso);
    ...
    describe('filtre par text', () => {
      ...
      test('recherche par identifiantReferentiel (cae_18)', async () => {
        ...
        const { data: result2 } = await caller.indicateurs.indicateurs.list({
          collectiviteId: 1,
          filters: { text: 'cae18' },
        });
  ...
  describe('tri', () => {
    ...
    test('tri alphabétique par titre', async () => {
      ...
      const { data: asc } = await caller.indicateurs.indicateurs.list({
        collectiviteId: 1,
      ...
      const { data: descData } = await caller.indicateurs.indicateurs.list({
        collectiviteId: 1,
        filters: { indicateurIds: [aId, zId] },
  ...
  describe('Payload structure and linked entities tests', () => {
    test('vérifie la structure complète du résultat avec pagination', async () => {
      ...
      result.forEach((indicateur) => {
        const toCheck = indicateurListItemSchema.safeParse(indicateur);
