...
@Injectable()
export class ListIndicateursRepository {
  constructor(
  ...
  private getIndicateurDefinitionPilotesQuery(collectiviteId: number) {
    return this.databaseService.db
      ...
      .where(eq(indicateurPiloteTable.collectiviteId, collectiviteId))
  ...
  private getIndicateurDefinitionServicesQuery(collectiviteId: number) {
    return this.databaseService.db
      ...
      .where(eq(indicateurServiceTagTable.collectiviteId, collectiviteId))
  ...
  async listIndicateurs({
    ...
    filters,
    queryOptions,
  }: ListIndicateursInput) {
    //  First get all subqueries agnostic of collectiviteId

    const indicateurThematiques =
      this.listPlatformDefinitionsRepository.getThematiquesSubQuery();
    const indicateurMesures =
      this.listPlatformDefinitionsRepository.getMesuresSubQuery();
  ...
  async getPath(indicateurId: number) {
  ...
  async getFavorisCount(collectiviteId: number) {
  ...
  async getPersonnalisesCount(collectiviteId: number) {
    const rows = await this.databaseService.db
      .select({ count: count() })
      .from(indicateurDefinitionTable)
  ...
  async getMesIndicateursCount(collectiviteId: number, userId: string) {
    const rows = await this.databaseService.db
      ...
      .where(
        and(
          eq(indicateurPiloteTable.userId, userId),
