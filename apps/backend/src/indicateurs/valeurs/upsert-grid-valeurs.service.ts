...
export class UpsertGridValeursService {
  ...
  async upsertGridValeurs(
    ...
    try {
      ...
      >(async (tx) => {
        try {
          // Le verrou de définition ferme la course avec un changement de
          // périodicité : janvier annuel et janvier mensuel partagent la même
          // date SQL et ne peuvent pas être distingués par le trigger seul.
