...
import { createIndicateurPerso } from '../definitions.fixture';
...
describe('IndicateurDefinitionThematiqueRouter', () => {
  ...
  test('list existing thematiques associated with an indicateur', async () => {
    ...
    const indicateurId = await createIndicateurPerso({
      ...
      indicateurData: {
        ...
        thematiques: [3], // assume that the thematique 3 exists
      },
    });

    const [indicateur] = await caller.indicateurs.definitions.list({
  ...
  test('list, update, delete thematiques associated with an indicateur and a collectivite', async () => {
    ...
    const [indicateurBefore] = await caller.indicateurs.definitions.list({
      indicateurIds: [indicateurId],
      collectiviteId,
    });
    ...
    const [indicateurAfter] = await caller.indicateurs.definitions.list({
      indicateurIds: [indicateurId],
      collectiviteId,
    });
    ...
    const [indicateurEmpty] = await caller.indicateurs.definitions.list({
      indicateurIds: [indicateurId],
      collectiviteId,
    });
