...
import {
  indicateurServiceTagFixture1,
  indicateurServiceTagFixture2,
} from '../../shared/fixtures/indicateur-service-tag.fixture';
import { indicateurFixture } from '../../shared/fixtures/indicateur.fixture';
import {
  serviceTagFixture1,
  serviceTagFixture2,
  serviceTagFixture3,
} from '../../shared/fixtures/service-tag.fixture';
...
describe('IndicateurServiceTagRouter', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      await databaseService.db
        ...
        .values(serviceTagFixture1);
      await databaseService.db
        .insert(serviceTagTable)
        .values(serviceTagFixture2);
      await databaseService.db
        .insert(serviceTagTable)
        .values(serviceTagFixture3);
      ...
      await databaseService.db
        ...
        .values(indicateurServiceTagFixture1);
      await databaseService.db
        .insert(indicateurServiceTagTable)
        .values(indicateurServiceTagFixture2);
    } catch (error) {
  ...
  describe('List indicateur service pilote', () => {
    test('should list all service pilote associated with an indicateur and a collectivete', async () => {
      ...
      const servicePilotes =
        await caller.indicateurs.definitions.indicateursServicesPilotes.list({
          indicateurId,
          collectiviteId,
        });
