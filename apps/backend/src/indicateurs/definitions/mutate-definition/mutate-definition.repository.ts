...
type UpdatePersonalizedDefinitionFields = PersonalizedDefinitionKey &
  Partial<Pick<IndicateurDefinition, 'titre' | 'unite' | 'periodicite'>>;
...
export class MutateDefinitionRepository {
  ...
  async hasValeurs(indicateurId: number, tx: Transaction): Promise<boolean> {
    const [existingValeur] = await tx
      .select({ id: indicateurValeurTable.id })
      .from(indicateurValeurTable)
      .where(eq(indicateurValeurTable.indicateurId, indicateurId))
      .limit(1);

    return Boolean(existingValeur);
  }
