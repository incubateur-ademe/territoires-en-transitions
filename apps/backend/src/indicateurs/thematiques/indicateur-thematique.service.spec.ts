...
describe('IndicateurThematiqueService', () => {
  ...
  describe('List indicateur thematique', () => {
    test('should list all thematiques associated with an indicateur', async () => {
      ...
      const serviceThematiques = await caller.indicateurs.thematiques.list({
        indicateurId,
        collectiviteId,
      })

      expect(serviceThematiques).toHaveLength(2);
      expect(serviceThematiques[0]).equal(thematique2.id);
      expect(serviceThematiques[1]).equal(thematique1.id);
  ...
  describe('Upsert indicateur service thematique', () => {
    test('should upsert a service thematique associated with an indicateur and a collectivite and modified the modifiedBy field', async () => {
      ...
      await caller.indicateurs.thematiques.upsert({
        indicateurId,
        collectiviteId,
      ...
      expect(serviceThematiques).toHaveLength(2);
      expect(serviceThematiques[0]).equal(thematique3.id);
      expect(serviceThematiques[1]).equal(thematique1.id);
