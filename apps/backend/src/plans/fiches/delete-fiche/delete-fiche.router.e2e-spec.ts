...
const ficheId = ficheActionFixture.id as number;
...
describe('DeleteFicheRouter', () => {
  ...
  it('marque la fiche et ses sous-fiches comme étant supprimées (soft delete)', async () => {
    const caller = router.createCaller({ user: yoloDodo });
    await db.db.insert(ficheActionTable).values({
      ...ficheActionFixture,
      collectiviteId,
    ...
    const [sousFiche] = await db.db
      ...
      .values({
        collectiviteId,
        titre: 'Sous-fiche à supprimer',
        parentId: ficheActionFixture.id,
      })
      .returning();
  ...
  it('supprime la fiche et ses sous-fiches', async () => {
    const caller = router.createCaller({ user: yoloDodo });
    await db.db.insert(ficheActionTable).values({
      ...ficheActionFixture,
      collectiviteId,
