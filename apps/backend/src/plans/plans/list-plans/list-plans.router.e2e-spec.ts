  ...
  describe('Lister les plans - Cas de succès', () => {
    ...
    test('Lister avec succès un plan unique', async () => {
      ...
      onTestFinished(async () => {
        ...
        await cleanupCaller.plans.plans.deletePlan({ planId });
      });
    ...
    test('Lister avec succès plusieurs plans', async () => {
      ...
      onTestFinished(async () => {
        const cleanupCaller = router.createCaller({ user: editorUser });
        await cleanupCaller.plans.plans.deletePlan({ planId: plan1.id });
        await cleanupCaller.plans.plans.deletePlan({ planId: plan2.id });
        await cleanupCaller.plans.plans.deletePlan({ planId: plan3.id });
      });
    ...
    test('Lister avec succès les plans triés par nom croissant', async () => {
      ...
      onTestFinished(async () => {
        const cleanupCaller = router.createCaller({ user: editorUser });
        await cleanupCaller.plans.plans.deletePlan({ planId: planA.id });
        await cleanupCaller.plans.plans.deletePlan({ planId: planB.id });
        await cleanupCaller.plans.plans.deletePlan({ planId: planC.id });
      });
