...
import { InvalidAxeHierarchy, InvalidPlanData } from './plan.errors';
import {
...
export function validatePlanCreationData(
  ...
  | { valid: false; error: InvalidPlanData } {
...
export function findParentAxeId(
  ...
  planId: number
): Result<number, InvalidAxeHierarchy> {
  ...
  if (parentId === undefined) {
    return failure(new InvalidAxeHierarchy(childPath, parentPath));
  }
