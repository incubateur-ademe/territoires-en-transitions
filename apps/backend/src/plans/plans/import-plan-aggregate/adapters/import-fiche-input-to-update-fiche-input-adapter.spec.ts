...
describe('importFicheInputToUpdateFicheInput', () => {
  ...
  it('should map instance gouvernance from resolved entities to fiche input', () => {
    const fiche: ImportFicheInput = {
      titre: 'Fiche avec instances de gouvernance',
    ...
    expect(result.instanceGouvernance).toEqual([
      { id: 1, nom: 'Comité de pilotage' },
      { id: 2, nom: 'Conseil municipal' },
    ]);
  });

  it('should handle empty instance gouvernance array', () => {
    const fiche: Partial<ImportFicheInput> = {
      titre: 'Fiche sans instances de gouvernance',
      instanceGouvernance: [],
    };
  ...
  it('should map all other fiche fields correctly', () => {
    const fiche: Partial<ImportFicheInput> = {
    ...
    expect(result.statut).toBe('non_commence');
