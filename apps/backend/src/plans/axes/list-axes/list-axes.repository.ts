...
import { AxeNode, FlatAxe, flatAxeSchema } from './flat-axe.schema';
...
export class ListAxesRepository {
  ...
  async listChildrenRecursively(
    input: ListAxesInput,
    tx?: Transaction
  ): Promise<MethodResult<AxeNode[], ListAxesError>> {
    try {
      ...
      SELECT id, nom, ancestors, depth, sort_path
      FROM parents
      ORDER BY naturalsort(sort_path);
    `);

      return this.sanitizeAxes(result.rows);
    } catch (error) {
      this.logger.error(
  ...
  ): MethodResult<AxeNode[], ListAxesError> {
