...
> {
   ...
   * à chaque sauvegarde de l'axe (et pour poser created_by sur les relations
   * effectivement créées : cette table n'a plus de défaut `auth.uid()`,
   * valeur toujours null sous connexion Drizzle — cf
   * upsert-plan.repository.ts:58-60 — created_by est donc obligatoire et posé
   * applicativement).
   */
