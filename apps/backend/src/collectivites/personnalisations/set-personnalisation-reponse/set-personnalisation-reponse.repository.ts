import { Injectable, Logger } from '@nestjs/common';
import { AuthenticatedUser } from '@tet/backend/users/models/auth.models';
import { Transaction } from '@tet/backend/utils/database/transaction.utils';
import { Result } from '@tet/backend/utils/result.type';
import {
  PersonnalisationReponse,
  PersonnalisationReponseValue,
} from '@tet/domain/collectivites';
import { and, eq, sql } from 'drizzle-orm';
import { DatabaseError } from 'pg';
import { justificationTable } from '../models/justification.table';
import { questionTable } from '../models/question.table';
import { reponseBinaireTable } from '../models/reponse-binaire.table';
import { reponseChoixTable } from '../models/reponse-choix.table';
import { reponseProportionTable } from '../models/reponse-proportion.table';
import {
  JustificationSnapshot,
  ReponseBinaireSnapshot,
  ReponseChoixSnapshot,
  ReponseProportionSnapshot,
} from './set-personnalisation-reponse-historique.repository';
import {
  SetPersonnalisationReponseError,
  SetPersonnalisationReponseErrorEnum,
} from './set-personnalisation-reponse.errors';
import { SetPersonnalisationReponseInput } from './set-personnalisation-reponse.input';

type ReponseSnapshotByQuestionType = {
  binaire: ReponseBinaireSnapshot;
  choix: ReponseChoixSnapshot;
  proportion: ReponseProportionSnapshot;
};

export type SetPersonnalisationReponseWriteData =
  | {
      questionType: 'binaire';
      reponse: PersonnalisationReponseValue;
      newRow: ReponseBinaireSnapshot;
      oldRow: ReponseBinaireSnapshot | null;
    }
  | {
      questionType: 'choix';
      reponse: PersonnalisationReponseValue;
      newRow: ReponseChoixSnapshot;
      oldRow: ReponseChoixSnapshot | null;
    }
  | {
      questionType: 'proportion';
      reponse: PersonnalisationReponseValue;
      newRow: ReponseProportionSnapshot;
      oldRow: ReponseProportionSnapshot | null;
    };

export type SetPersonnalisationJustificationWriteData = {
  justification: PersonnalisationReponse['justification'];
  newRow: JustificationSnapshot;
  oldRow: JustificationSnapshot | null;
};

@Injectable()
export class SetPersonnalisationReponseRepository {
  private readonly logger = new Logger(
    SetPersonnalisationReponseRepository.name
  );

  constructor() {}

  async setReponse(
    input: SetPersonnalisationReponseInput,
    tx: Transaction
  ): Promise<
    Result<SetPersonnalisationReponseWriteData, SetPersonnalisationReponseError>
  > {
    const { collectiviteId, questionId, reponse } = input;

    try {
      // récupère le type de la question
      const [question] = await tx
        .select({ type: questionTable.type })
        .from(questionTable)
        .where(eq(questionTable.id, questionId))
        .limit(1);

      if (!question) {
        this.logger.log(
          `Erreur de lecture du type de la question ${questionId}`
        );
        return {
          success: false,
          error: SetPersonnalisationReponseErrorEnum.QUESTION_NOT_FOUND,
        };
      }

      // insère ou met à jour la réponse selon le type de question
      let writeData: SetPersonnalisationReponseWriteData;

      if (question.type === 'binaire') {
        if (reponse !== null && typeof reponse !== 'boolean') {
          this.logger.error(
            `La réponse pour une question binaire doit être un booléen ou null (${questionId}: ${reponse})`
          );
          return {
            success: false,
            error: SetPersonnalisationReponseErrorEnum.INVALID_RESPONSE_TYPE,
          };
        }
        const oldRow = await tx
          .select()
          .from(reponseBinaireTable)
          .where(
            and(
              eq(reponseBinaireTable.collectiviteId, collectiviteId),
              eq(reponseBinaireTable.questionId, questionId)
            )
          )
          .for('update')
          .then((rows) => rows[0] ?? null);

        const result = await tx
          .insert(reponseBinaireTable)
          .values({
            collectiviteId,
            questionId,
            reponse,
          })
          .onConflictDoUpdate({
            target: [
              reponseBinaireTable.collectiviteId,
              reponseBinaireTable.questionId,
            ],
            set: {
              reponse: sql.raw(`excluded.${reponseBinaireTable.reponse.name}`),
              modifiedAt: sql`CURRENT_TIMESTAMP`,
            },
          })
          .returning()
          .then((rows) => rows[0]);

        writeData = {
          questionType: 'binaire',
          reponse: result.reponse,
          newRow: result as ReponseSnapshotByQuestionType['binaire'],
          oldRow: oldRow as ReponseSnapshotByQuestionType['binaire'] | null,
        };
      } else if (question.type === 'proportion') {
        if (
          reponse !== null &&
          (typeof reponse !== 'number' ||
            !Number.isFinite(reponse) ||
            reponse < 0 ||
            reponse > 1)
        ) {
          this.logger.error(
            `La réponse pour une question proportion doit être un nombre fini compris entre 0 et 1 (${questionId}: ${reponse})`
          );
          return {
            success: false,
            error: SetPersonnalisationReponseErrorEnum.INVALID_RESPONSE_TYPE,
          };
        }
        const oldRow = await tx
          .select()
          .from(reponseProportionTable)
          .where(
            and(
              eq(reponseProportionTable.collectiviteId, collectiviteId),
              eq(reponseProportionTable.questionId, questionId)
            )
          )
          .for('update')
          .then((rows) => rows[0] ?? null);

        const result = await tx
          .insert(reponseProportionTable)
          .values({
            collectiviteId,
            questionId,
            reponse,
          })
          .onConflictDoUpdate({
            target: [
              reponseProportionTable.collectiviteId,
              reponseProportionTable.questionId,
            ],
            set: {
              reponse: sql.raw(
                `excluded.${reponseProportionTable.reponse.name}`
              ),
              modifiedAt: sql`CURRENT_TIMESTAMP`,
            },
          })
          .returning()
          .then((rows) => rows[0]);

        writeData = {
          questionType: 'proportion',
          reponse: result.reponse,
          newRow: result as ReponseSnapshotByQuestionType['proportion'],
          oldRow: oldRow as ReponseSnapshotByQuestionType['proportion'] | null,
        };
      } else if (question.type === 'choix') {
        if (reponse !== null && typeof reponse !== 'string') {
          this.logger.error(
            `La réponse pour une question à choix doit être une chaîne de caractères ou null (${questionId}: ${reponse})`
          );
          return {
            success: false,
            error: SetPersonnalisationReponseErrorEnum.INVALID_RESPONSE_TYPE,
          };
        }
        const oldRow = await tx
          .select()
          .from(reponseChoixTable)
          .where(
            and(
              eq(reponseChoixTable.collectiviteId, collectiviteId),
              eq(reponseChoixTable.questionId, questionId)
            )
          )
          .for('update')
          .then((rows) => rows[0] ?? null);

        const result = await tx
          .insert(reponseChoixTable)
          .values({
            collectiviteId,
            questionId,
            reponse,
          })
          .onConflictDoUpdate({
            target: [
              reponseChoixTable.collectiviteId,
              reponseChoixTable.questionId,
            ],
            set: {
              reponse: sql.raw(`excluded.${reponseChoixTable.reponse.name}`),
              modifiedAt: sql`CURRENT_TIMESTAMP`,
            },
          })
          .returning()
          .then((rows) => rows[0]);

        writeData = {
          questionType: 'choix',
          reponse: result.reponse,
          newRow: result as ReponseSnapshotByQuestionType['choix'],
          oldRow: oldRow as ReponseSnapshotByQuestionType['choix'] | null,
        };
      } else {
        this.logger.log(`Type de question non supporté ${question.type}`);
        return {
          success: false,
          error: SetPersonnalisationReponseErrorEnum.UNSUPPORTED_QUESTION_TYPE,
        };
      }

      return {
        success: true,
        data: writeData,
      };
    } catch (error) {
      this.logger.error(
        `Erreur d'enregistrement de la réponse (${error}) ${collectiviteId} ${questionId}`
      );
      return {
        success: false,
        error: SetPersonnalisationReponseErrorEnum.DATABASE_ERROR,
      };
    }
  }

  async setJustification(
    input: SetPersonnalisationReponseInput,
    user: AuthenticatedUser,
    tx: Transaction
  ): Promise<
    Result<
      SetPersonnalisationJustificationWriteData | null,
      SetPersonnalisationReponseError
    >
  > {
    const { collectiviteId, questionId, justification } = input;
    if (justification === undefined) {
      return { success: true, data: null };
    }

    try {
      const oldRow = await tx
        .select()
        .from(justificationTable)
        .where(
          and(
            eq(justificationTable.collectiviteId, collectiviteId),
            eq(justificationTable.questionId, questionId)
          )
        )
        .for('update')
        .then((rows) => rows[0] ?? null);

      const result = await tx
        .insert(justificationTable)
        .values({
          collectiviteId,
          questionId,
          texte: justification,
          modifiedAt: sql`CURRENT_TIMESTAMP`,
          modifiedBy: user.id,
        })
        .onConflictDoUpdate({
          target: [
            justificationTable.collectiviteId,
            justificationTable.questionId,
          ],
          set: {
            texte: sql.raw(`excluded.${justificationTable.texte.name}`),
            modifiedBy: sql.raw(`excluded.${justificationTable.modifiedBy.name}`),
            modifiedAt: sql`CURRENT_TIMESTAMP`,
          },
        })
        .returning()
        .then((rows) => rows[0]);

      return {
        success: true,
        data: {
          justification: result.texte,
          newRow: result,
          oldRow,
        },
      };
    } catch (error) {
      this.logger.error(
        `Erreur d'enregistrement de la justification (${
          (error as DatabaseError).message
        } - ${(error as DatabaseError).cause}) ${collectiviteId} ${questionId}`
      );

      return {
        success: false,
        error: SetPersonnalisationReponseErrorEnum.DATABASE_ERROR,
      };
    }
  }
}
