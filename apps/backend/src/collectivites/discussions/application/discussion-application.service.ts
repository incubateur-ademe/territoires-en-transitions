...
export class DiscussionApplicationService {
  ...
  async deleteDiscussionMessage(
    ...
    const { collectiviteId, messageId } = input;
    const hasPermission = await this.permissionService.isAllowed(
      user,
      PermissionOperationEnum['COLLECTIVITES.LECTURE'],
      ResourceType.COLLECTIVITE,
    ...
    const discussionMessageResult =
      await this.discussionDomainService.deleteDiscussionMessage(messageId);
    return discussionMessageResult;
  }
  ...
  async updateDiscussionMessage(
    ...
    if (hasSupportRole || hasADMERole) {
      this.logger.error(
        `L'utilisateur ${user.id} avec le rôle Support ou ADEME ne peut pas mettre à jour le message de discussion sur la collectivité ${collectiviteId}`
      );
      return {
        success: false,
        error: DiscussionErrorEnum.UNAUTHORIZED,
