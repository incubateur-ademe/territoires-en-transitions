...
describe('DiscussionRouter', () => {
  ...
  describe('updateMessage', () => {
    test('should update a discussion message successfully', async () => {
      const input = {
        messageId: 1,
        collectiviteId: 1,
        message: 'Updated message',
      ...
      (
        discussionApplicationService.updateDiscussionMessage as any
      ).mockResolvedValue(successResult);

      const updateMessageHandler = router.router.updateMessage;
      const result = await updateMessageHandler({
        input,
        ctx: { user: mockUser },
      } as any);

      expect(result).toEqual(mockDiscussionMessage);
      expect(
        discussionApplicationService.updateDiscussionMessage
      ).toHaveBeenCalledWith(input, mockUser);
    });

    test('should throw TRPC error when update message is unauthorized', async () => {
      const input = {
        messageId: 1,
        collectiviteId: 1,
        message: 'Updated message',
      ...
      await expect(
        updateMessageHandler({ input, ctx: { user: mockUser } } as any)
      ).rejects.toMatchObject({
        code: 'FORBIDDEN',
        message: "Vous n'avez pas les permissions nécessaires",
      });
    ...
    test('should throw TRPC error when discussion message is not found', async () => {
      const input = {
        messageId: 999,
        collectiviteId: 1,
        message: 'Updated message',
