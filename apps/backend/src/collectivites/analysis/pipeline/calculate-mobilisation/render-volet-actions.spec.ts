...
describe('renderVoletActions', () => {
      ...
      withAmenagement([
        {
          ...
          titre: 'Piste',
          description:
            'Un\ndeux\rtrois\u2028quatre\u0085cinq\u2029six\vsept\fhuit',
        },
      ])
    );

    const ficheLines = text
      .split('\n')
      .filter((line) => line.startsWith('42 |'));

    expect({
      lineCount: ficheLines.length,
      keepsWholeText: ficheLines[0].includes('huit'),
    }).toEqual({ lineCount: 1, keepsWholeText: true });
  });
  ...
  it("empêche une fiche de forger le séparateur d'une autre action", () => {
    ...
    const [, afterFirstSeparator] = text.split('42 | Piste : ');

    expect(afterFirstSeparator.split('\n')[0]).not.toContain('|');
  });
