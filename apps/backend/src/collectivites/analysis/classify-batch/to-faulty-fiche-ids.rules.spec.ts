...
describe('retry-on-failure', () => {
  ...
  it('désigne tout le lot quand la limite de débit du LLM est atteinte', () => {
    expect(
      toFaultyFicheIds({ failure: { kind: 'rate_limited' }, fiches: batch })
    ).toEqual([11, 12, 13]);
  });

  it('désigne tout le lot pour une réponse tronquée', () => {
    expect(
      toFaultyFicheIds({ failure: { kind: 'truncated' }, fiches: batch })
    ).toEqual([11, 12, 13]);
  });

  it('désigne tout le lot pour une réponse en JSON invalide', () => {
    expect(
      toFaultyFicheIds({
  ...
  it("désigne tout le lot pour une erreur de l'API", () => {
    expect(
      toFaultyFicheIds({
  ...
  it('désigne tout le lot pour un lot vide', () => {
    expect(
      toFaultyFicheIds({ failure: { kind: 'empty_batch' }, fiches: batch })
    ).toEqual([11, 12, 13]);
  });

  it('désigne tout le lot pour un lot trop grand', () => {
    expect(
      toFaultyFicheIds({
  ...
  it('désigne tout le lot pour une réponse vide', () => {
    expect(
      toFaultyFicheIds({ failure: { kind: 'empty_response' }, fiches: batch })
    ).toEqual([11, 12, 13]);
  });

  it('désigne tout le lot pour un index hors du lot', () => {
    expect(
      toFaultyFicheIds({
  ...
  it('désigne tout le lot pour un enjeu inconnu', () => {
    expect(
      toFaultyFicheIds({
  ...
  it("désigne la fiche de l'index en double", () => {
    expect(
      toFaultyFicheIds({
  ...
  it("désigne la fiche d'une abstention contradictoire", () => {
    expect(
      toFaultyFicheIds({
  ...
  it("désigne la fiche d'une abstention non déclarée", () => {
    expect(
      toFaultyFicheIds({
  ...
  it('désigne les fiches des index manquants', () => {
    expect(
      toFaultyFicheIds({
