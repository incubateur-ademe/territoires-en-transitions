...
export const isAdeme = (user: UserWithCollectiviteAccesses) => {
  return user.email?.includes('@ademe.fr') ?? false;
};

export const canCreateDiscussion = (user: UserWithCollectiviteAccesses) => {
  return !user.isSupport && !isAdeme(user);
};
