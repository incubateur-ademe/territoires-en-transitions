...
describe('isPremiereEtoileDemandeEnCours', () => {
  it('est vrai pour une demande labellisation 1ère étoile envoyée', () => {
    expect(
      isPremiereEtoileDemandeEnCours(
        buildParcours({
  ...
  it('est faux quand aucune demande n\'a été envoyée', () => {
    expect(
      isPremiereEtoileDemandeEnCours(buildParcours({ status: 'non_demandee' }))
    ).toBe(false);
  });

  it('est faux quand le parcours est absent', () => {
    expect(isPremiereEtoileDemandeEnCours(null)).toBe(false);
  });
});
