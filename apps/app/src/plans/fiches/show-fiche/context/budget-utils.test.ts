...
describe('budget-utils', () => {
  describe('transformFicheBudgetsToBudgetPerYear', () => {
    it('should handle all transformation scenarios', () => {
      ...
      const result = transformFicheBudgetsToBudgetPerYear(
        allBudgets,
        'investissement'
      );
      ...
      const fonctResult = transformFicheBudgetsToBudgetPerYear(
        allBudgets,
        'fonctionnement'
      );
      ...
      expect(
        transformFicheBudgetsToBudgetPerYear([], 'investissement')
      ).toEqual([]);
    });
  });
  ...
  describe('transformFicheBudgetsToBudgetSummary', () => {
    it('should handle all transformation scenarios', () => {
      ...
      const investResult = transformFicheBudgetsToBudgetSummary(
        allBudgets,
        'investissement'
      );
      ...
      const fonctResult = transformFicheBudgetsToBudgetSummary(
        allBudgets,
        'fonctionnement'
      );
      expect(fonctResult).toEqual({
      ...
      expect(
        transformFicheBudgetsToBudgetSummary(
          allBudgets.filter(
            (b) => b.type === 'investissement' && b.annee !== null
          ),
          'investissement'
        )
      ).toBeNull();
      ...
      // Test with empty array
      expect(
        transformFicheBudgetsToBudgetSummary([], 'investissement')
      ).toBeNull();
    });
  });
  ...
  describe('transformBudgetToFicheBudgetCreate', () => {
    it('should handle all transformation scenarios', () => {
      ...
      const fullResult = transformBudgetToFicheBudgetCreate(
        fullBudget,
        100,
        'investissement'
      );
      ...
      const summaryResult = transformBudgetToFicheBudgetCreate(
        summaryBudget,
        100,
        'fonctionnement'
      );
      ...
      // Test with only HT
      const htOnlyBudget: BudgetPerYear = {
        year: 2024,
        montant: 50000,
        depense: 45000,
      };
      const htResult = transformBudgetToFicheBudgetCreate(
        htOnlyBudget,
        100,
        'investissement'
      );
      expect(htResult).toHaveLength(2);
      expect(htResult.find((b) => b.unite === 'HT')).toBeDefined();
      expect(htResult.find((b) => b.unite === 'ETP')).toBeDefined();

      // Test with only ETP
      const etpOnlyBudget: BudgetPerYear = {
        year: 2024,
        etpPrevisionnel: 3.0,
        etpReel: 2.5,
      };
      const etpResult = transformBudgetToFicheBudgetCreate(
        etpOnlyBudget,
        100,
        'investissement'
      );
      expect(etpResult).toHaveLength(2);
      expect(etpResult.find((b) => b.unite === 'ETP')).toBeDefined();
      expect(etpResult.find((b) => b.unite === 'HT')).toBeDefined();

      // Test with only previsionnel values
      const prevBudget: BudgetPerYear = {
        year: 2024,
        montant: 50000,
        etpPrevisionnel: 2.5,
      };
      const prevResult = transformBudgetToFicheBudgetCreate(
        prevBudget,
        100,
        'investissement'
      );
      ...
      const reelBudget: BudgetPerYear = {
        year: 2024,
        depense: 45000,
        etpReel: 2.0,
      };
      const reelResult = transformBudgetToFicheBudgetCreate(
        reelBudget,
        100,
        'investissement'
      );
      expect(reelResult).toHaveLength(2);
      expect(reelResult.every((b) => b.budgetPrevisionnel === undefined)).toBe(
        true
      );

      // Test with all undefined values (should return array with empty values for each key)
      const emptyBudget: BudgetPerYear = { year: 2024 };
      const emptyResult = transformBudgetToFicheBudgetCreate(
        emptyBudget,
        100,
        'investissement'
      );
      expect(emptyResult).toHaveLength(2);
      expect(emptyResult.every((b) => b.budgetPrevisionnel === undefined)).toBe(
        true
      );
      expect(emptyResult.every((b) => b.budgetReel === undefined)).toBe(true);

      // Test ID preservation
      const idBudget: BudgetPerYear = {
        year: 2024,
        montant: 50000,
        etpPrevisionnel: 2.5,
        htBudgetId: 10,
      ...
      const idResult = transformBudgetToFicheBudgetCreate(
        idBudget,
        100,
        'investissement'
      );
