...
const useSignupStep1 = (
  ...
  const validationSchema = z
    ...
    .superRefine((data, ctx) => {
      ...
      if (!isScoreStrongEnough(data.password, [data.email])) {
        ctx.addIssue({
          code: 'custom',
          path: ['password'],
...
const SignupStep1Form = (
  ...
  const isEmailAlreadyExists = error === USER_ALREADY_EXISTS_ERROR;
    ...
    <form className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmitForm)}>
      <Field
        ...
        message={
          ...
          (isEmailAlreadyExists ? appLabels.authEmailDejaAssocie : undefined)
        }
      >
