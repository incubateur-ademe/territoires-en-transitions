...
import { z } from 'zod';
import { ResendFunction, VerifyType } from '../verify-otp';
  ...
  return (
    <div>
      <Button variant="underlined" onClick={() => handleResend(!opened)}>
        {buttonText || appLabels.authPasRecuMessage}
      </Button>

      {opened && (
        <div className="my-4 flex flex-col">
          <p>
            {appLabels.authVerifierDossierSpam} <i>{appLabels.authSpam}</i>{' '}
            {appLabels.authDeVotreMessagerie}
            {appLabels.authNouvelEnvoiMessageAdresse}
          </p>
          <Input
