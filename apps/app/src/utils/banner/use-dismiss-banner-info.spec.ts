...
const MESSAGE_VERSION = '2026-08-19T10:00:00.000Z';
const NOUVEAU_MESSAGE_VERSION = '2026-08-19T11:00:00.000Z';
...
describe('useDismissBannerInfo', () => {
  ...
  it("affiche la bannière tant que rien n'a été fermé", () => {
    const { result } = renderHook(() => useDismissBannerInfo(MESSAGE_VERSION));
  ...
  it('garde la bannière masquée au rechargement', () => {
    const { result: premierChargement } = renderHook(() =>
      useDismissBannerInfo(MESSAGE_VERSION)
    );
  ...
  it('réaffiche la bannière quand un nouveau message est publié', () => {
    localStorage.setItem(STORAGE_KEY, MESSAGE_VERSION);

    const { result } = renderHook(() =>
      useDismissBannerInfo(NOUVEAU_MESSAGE_VERSION)
    );
  ...
  it("réaffiche la bannière quand un nouveau message remplace celui qui vient d'être fermé", () => {
    const { result, rerender } = renderHook(
      ({ messageVersion }) => useDismissBannerInfo(messageVersion),
      { initialProps: { messageVersion: MESSAGE_VERSION } }
    );
    act(() => result.current.dismiss());
    expect(result.current.isVisible).toBe(false);

    rerender({ messageVersion: NOUVEAU_MESSAGE_VERSION });
