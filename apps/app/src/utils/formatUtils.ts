...
export const getFormattedNumber = (nb: number) => {
  return nb.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ').replace('.', ',');
};
...
// Parse budget values, handling French number formatting (comma as decimal separator)
export const parseBudgetNumber = (value: string | null | undefined): number | null => {
