  ...
  return useMutation(
    trpc.indicateurs.definitions.indicateursServicesPilotes.upsert.mutationOptions({
      onSuccess: (_, variables) => {
        ...
        queryClient.invalidateQueries({
          queryKey: trpc.indicateurs.definitions.indicateursServicesPilotes.list.queryKey({
            collectiviteId,
            indicateurId
          }),
        });
...
export const useIndicateurServices = (indicateurId: number) => {
  ...
  return useQuery(trpc.indicateurs.definitions.indicateursServicesPilotes.list.queryOptions({
    collectiviteId,
    indicateurId
  }));
};
