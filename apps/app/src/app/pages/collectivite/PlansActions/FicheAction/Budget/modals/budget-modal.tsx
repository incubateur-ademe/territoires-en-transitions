...
import DetailedBudgetForm from '@/app/app/pages/collectivite/PlansActions/FicheAction/Budget/modals/detailed-budget-form';
...
import { parseBudgetNumber } from '@/app/utils/formatUtils';
import {
  Alert,
  ButtonGroup,
  Checkbox,
...
const BudgetModal = ({ openState, fiche, type, budgets }: BudgetModalProps) => {
  ...
  const handleSave = () => {
    ...
    // clean data
    const cleanedBudgetData = budgetData.map((elt) => ({
      ...elt,
      budgetPrevisionnel:
        parseBudgetNumber(elt.budgetPrevisionnel)?.toString() ?? null,
      budgetReel: parseBudgetNumber(elt.budgetReel)?.toString() ?? null,
    }));

    const htBudgets = cleanedBudgetData.filter((elt) => elt.unite === 'HT');
    const etpBudgets = cleanedBudgetData.filter((elt) => elt.unite === 'ETP');
