...
import { ListFichesRequestFilters, NotesOption } from '@tet/domain/plans';
    ...
    queryFn: async () => {
      ...
      mergedFiltersKeys.forEach((key) => {
        ...
        if (key === 'utilisateurPilotes') {
          ...
          badgeValues.push(
            NOTES_PROPERTIES[mergedFilters[key] as NotesOption].label
          );
        } else if (
          key === 'anneesNotes' &&
          mergedFilters[key] &&
          ...
          badgeValues.push('Sans tag personnalisés');
