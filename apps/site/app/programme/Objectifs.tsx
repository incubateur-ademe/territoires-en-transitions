...
type ObjectifsProps = {
  intro: { titre: string; description?: string };
  objectifs: { titre: string; contenu: VignetteAvecDetails[] | null };
};

const Objectifs = ({ intro, objectifs }: ObjectifsProps) => {
  const [openedContent, setOpenedContent] = useState<number | undefined>();
  const { titre, contenu } = objectifs;

  return (
    <>
      <Section containerClassName="max-md:!py-6 md:!pt-14 md:!pb-10">
        <h1 className="text-center text-primary-10 text-3xl mb-0">
          {intro.titre}
        </h1>
        <p className="text-center text-primary-10 text-xl">
          {intro.description}
        </p>
      </Section>
      {contenu && contenu.length ? (
        <CardsSection
          ...
          cardsList={
            <CardsWrapper
              cols={contenu.length}
              className="max-md:gap-4 gap-8 mt-3"
            >
              {contenu.map((c, index) => (
                <Fragment key={index}>
                  <Card
