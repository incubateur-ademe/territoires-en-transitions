...
const baseFicheAction: FicheWithRelationsAndCollectivite = {
  id: 10108,
  parentId: null,
  createdBy: {
  ...
  libreTags: null,
  plans: [{ id: 8017, nom: 'PCAET 2022-2028', collectiviteId, type: 'PCAET' }],
  titre:
...
describe('CommunsFicheActionMapper', () => {
  test('Standard mapping', async () => {
    ...
    const communProjet = mapper.map(baseFicheAction);
    ...
    expect(communProjet).toMatchObject({
      ...
      plans: [{ externalId: '8017', nom: 'PCAET 2022-2028', type: 'PCAET' }],
    });
  });

  test('Maps parentId to parentExternalId', async () => {
    ...
    const mapper = new CommunsFicheActionMapper();
    const result = mapper.map(ficheAction);

    expect(result?.parentExternalId).toBe('9000');
    expect(result?.plans).toBeUndefined();
  });

  test('Sends undefined for parentExternalId and plans when both are null', async () => {
    const ficheAction: FicheWithRelationsAndCollectivite = {
      ...baseFicheAction,
      parentId: null,
      plans: null,
    ...
    expect(result?.parentExternalId).toBeUndefined();
    expect(result?.plans).toBeUndefined();
  });
});
