# Consommation énergétique globale annuelle du territoire
```yaml
id: cae_2.a
identifiant: 2.a
unite: GWh

titre_long: Quantité d'énergie consommée par les activités et les habitants par an
obligation_cae: true
actions:
  - cae_1.1.1
programmes:
  - cae
  - pcaet
climat_pratic_ids:
  - strategie
thematiques:
  - energie_et_climat
```
## Description
Cet indicateur estime la consommation énergétique finale annuelle du territoire, selon les exigences réglementaires des PCAET (décret n°2016-849 du 28 juin 2016 et arrêté du 4 août 2016 relatifs au plan climat-air-énergie territorial).
L'indicateur est exprimé en GWh.




# Consommation énergétique annuelle du territoire par habitant 
```yaml
id: cae_2.b
identifiant: 2.b
unite: MWh/hab.an
actions:
  - cae_1.1.1
programmes:
  - cae
climat_pratic_ids:
  - strategie
```
## Description
Pour faciliter les comparaisons, l’indicateur est ramené au nombre d’habitants (population municipale selon l’INSEE). Préciser l'année de référence en commentaire. L'évaluation est basée sur l'évolution de l'indicateur.




# Consommation énergétique annuelle du territoire pour la chaleur et le rafraîchissement 
```yaml
id: cae_2.c
identifiant: 2.c
unite: GWh
actions:
  - cae_3.2.2
programmes:
  - cae
climat_pratic_ids:
  - energie
```
## Description
Décomposition par usage de l'indicateur 2a:

Consommation énergétique globale annuelle du territoire (GWh)

- utile aux calculs des taux de production ENR (dénominateur).




# Consommation énergétique annuelle du territoire pour l'électricité 
```yaml
id: cae_2.d
identifiant: 2.d
unite: GWh
actions:
  - cae_3.2.3
programmes:
  - cae
climat_pratic_ids:
  - energie
```
## Description
Décomposition par usage de l'indicateur 2a:

Consommation énergétique globale annuelle du territoire (GWh)

- utile aux calculs des taux de production ENR (dénominateur).




# Consommation énergétique du résidentiel 
```yaml
id: cae_2.e
identifiant: 2.e
unite: GWh
actions:
  - cae_1.2.4
programmes:
  - cae
  - pcaet
climat_pratic_ids:
  - batiments
```
## Description
Décomposition par secteur réglementaire de l'indicateur global 2a:

Consommation énergétique globale annuelle du territoire (GWh).




# Consommation énergétique du tertiaire
```yaml
id: cae_2.f
identifiant: 2.f
unite: GWh
actions:
  - cae_6.3.1
programmes:
  - cae
  - pcaet
climat_pratic_ids:
  - dev_eco
```
## Description
Décomposition par secteur réglementaire de l'indicateur global 2a:

Consommation énergétique globale annuelle du territoire (GWh).




# Consommation énergétique  du transport routier
```yaml
id: cae_2.g
identifiant: 2.g
unite: GWh
actions:
  - cae_1.2.2
programmes:
  - cae
  - pcaet
climat_pratic_ids:
  - mobilites
```
## Description
Décomposition par secteur réglementaire de l'indicateur global 2a:

Consommation énergétique globale annuelle du territoire (GWh).




# Consommation énergétique du secteur "autres transports" 
```yaml
id: cae_2.h
identifiant: 2.h
unite: GWh
actions:
  - cae_1.2.2
programmes:
  - cae
  - pcaet
climat_pratic_ids:
  - mobilites
```
## Description
Décomposition par secteur réglementaire de l'indicateur global 2a:

Consommation énergétique globale annuelle du territoire (GWh).




# Consommation énergétique de l'agriculture
```yaml
id: cae_2.i
identifiant: 2.i
unite: GWh
actions:
  - cae_6.4.1
programmes:
  - cae
  - pcaet
climat_pratic_ids:
  - agri_alim
```
## Description
Décomposition par secteur réglementaire de l'indicateur global 2a:

Consommation énergétique globale annuelle du territoire (GWh).




# Consommation énergétique des déchets 
```yaml
id: cae_2.j
identifiant: 2.j
unite: GWh
actions:
  - cae_1.2.3
programmes:
  - cae
  - pcaet
climat_pratic_ids:
  - dechets
```
## Description
Décomposition par secteur réglementaire de l'indicateur global 2a:

Consommation énergétique globale annuelle du territoire (GWh).




# Consommation énergétique de l'industrie hors branche énergie
```yaml
id: cae_2.k
identifiant: 2.k
unite: GWh
actions:
  - cae_6.3.1
programmes:
  - cae
  - pcaet
climat_pratic_ids:
  - dev_eco
```
## Description
Décomposition par secteur réglementaire de l'indicateur global 2a:

Consommation énergétique globale annuelle du territoire (GWh).




