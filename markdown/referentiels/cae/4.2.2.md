...
##### Mettre en place un outil de suivi du trafic et utiliser des données pour l’amélioration de la politique de mobilité 
```yaml
identifiant: 4.2.2.2.2
```
###### Description
L'outil de suivi du trafic doit être en temps réel dans l’idéal.
Il peut permettre l'évolution des tracés des voies bus (pour les collectivités traversées par des flux importants et/ou celles dotées de transports en commun).
