...
##### Attribuer une mission de conseil à des personnes ou entreprises externes spécialisées ou procèder à un recrutement en interne si les ressources humaines sont insuffisantes
```yaml
identifiant: 5.1.1.1.2
```
###### Description
Personnes ou entreprises externes spécialisées : par exemple  conseiller FAIRE, Agence de mobilité, Agence locale, association spécialisée... 
...
#### Actions
##### Aménager le poste du chef de projet climat-air-énergie pour lui permettre de se consacrer pleinement à sa mission et de travailler en transversalité et de manière légitime avec les autres services 
```yaml
identifiant: 5.1.1.3.1
```
###### Description
Positionnement stratégique dans l'organigramme ou lettre de mission par exemple

##### Disposer d'une équipe projet transversale, pilotée par le chef de projet, chargée d’assurer la mise en œuvre de la stratégie climat-air-énergie qui se réunit régulièrement et dynamise la démarche en interne
```yaml
identifiant: 5.1.1.3.2
```

##### Avoir des DGS, DGA et/ou DST qui sont impliqués et contribuent au portage et soutiennent le chef de projet 
```yaml
identifiant: 5.1.1.3.3
```
###### Description
Le portage et le soutien se fait notamment vis à vis de la transversalité et de la mobilisation des autres agents.
...
```
###### Description
Par exemple : les documents nécessaire pour réaliser l'état des lieux de la progression de la collectivité au regard du référentiel national ont été transmis sans trop de difficultés et dans les délais planifiés avec le conseiller ou la conseillère.
...
##### Améliorer la qualité de l'organisation en s'appuyant sur les retours des agents
```yaml
identifiant: 5.1.1.4.3
```

##### Compléter et améliorer les fiches de postes 
```yaml
identifiant: 5.1.1.4.4
```

##### Maintenir ou augmenter le nombre d’équivalents temps plein (ETP) dédiés à la mise en œuvre de la politique climat-air-énergie 
```yaml
