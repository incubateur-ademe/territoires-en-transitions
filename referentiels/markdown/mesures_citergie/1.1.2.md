...
##### Décision de confier une mission externe ou sollicitation de ressources humaines internes.
```yaml
id: 1.1.2.1.2
```
...
id: 1.1.2.1.4
```
art.179 LTECV, SCoT, PLU, PDM, PLH, Agenda 21, déclinaisons locales du PRSE, PPA, enquêtes ménages, Bilans Carbone®, audits énergétiques, études des acteurs locaux (Observatoire régional de l’énergie, Agence Régionale de l’Environnement - ARE, ALEC, AASQA, etc.).
...
#### Actions
##### Inventaire des émissions de GES et de polluants atmosphériques.
```yaml
id: 1.1.2.2.1
```
...
##### Estimation de la séquestration nette de dioxyde de carbone et de ses possibilités de développement.
```yaml
id: 1.1.2.2.2
```
Doit identifier au moins les sols agricoles et la forêt.

##### Analyse des bilans de consommation énergétique finale du territoire et du potentiel de réduction de celle-ci.
```yaml
id: 1.1.2.2.3
```
Avec distinction entre les besoins de chaleur, d'électricité et de carburant.
...
#### Actions
##### Prise en compte d'émissions indirectes.
```yaml
id: 1.1.2.4.1
```
Peuvent être liées à la fabrication des produits achetés et consommés par les acteurs du territoire
...
##### Le détail dans chaque domaine est d’une grande finesse.
```yaml
id: 1.1.2.4.3
```
...
```
#### Actions
##### Les évolutions constatées sont analysées.
```yaml
id: 1.1.2.5.1
```
